...
| `run-6-batch-size.mjs` | t350: the REAL pooled-particle lever (`--pool 3` = RELION 5's own GUI default, explicit user value wins; the t341 `--batch_size`/`--highres_limit` flags NEVER EXISTED in any RELION release and are now extinct — `highresLimit` rides as `--strict_highres_exp`, `scratchDir` as `--scratch_dir`) + the never-again guard (every emitted `--flag` ∈ the 161-option verified set transcribed from 3dem/relion 5.0-beta/5.0 sources) + the sbatch shape (one rank per GPU as the script's own `CF_RANKS` clamp + the t345 per-rank launcher, the t341 GPU-pin block, the t342 starved-card refusal block) |
