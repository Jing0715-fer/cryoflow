...
/* ------------------------------------------------------------------ */
/* t391 — the app shell (page.tsx's whole body, lifted verbatim)        */
/* ------------------------------------------------------------------ */
/* The home route is a THIN dynamic shell now (see src/app/page.tsx):
 * this component keeps every line of the application body — hooks,
 * canvas, panels, dialogs, keyboard maps — exactly as they were, only
 * its home moved. Why: the Next 16 dev server EAGERLY compiles the
 * entry page at boot, and this app's client graph (10k+ lines + radix +
 * charts) made that eager compile the single biggest allocation on a
 * small box — the API-only workloads (job runs, log streaming, the
 * sweep) died in its residual baseline. As its own chunk it compiles on
 * the FIRST BROWSER VISIT (when nothing else competes) and the boot
 * compile is the thin shell above it. The production build is unchanged
 * mechanically (standalone still emits everything); the first paint
 * gains a standard loading shell. */
...
// t391 — the dashboard is its own compile unit AND its own client chunk: it
// renders ONLY in the dashboard view, so paying its 2500-line graph (with the
// whole recharts surface) at page-compile time was pure loss — the home
// compile was the OOM ceiling on 4GB boxes and the initial bundle on the
// user's Windows machine carried charts it may never open. The lazy chunk
// compiles on first dashboard visit; the loading shell keeps the layout.
const ProjectDashboard = dynamic(
  () => import("@/components/workflow/project-dashboard").then((m) => m.ProjectDashboard),
  {
    ssr: false,
    loading: () => (
