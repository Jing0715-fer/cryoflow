...
/* ------------------------------------------------------------------ */
/* Sibling compare picker — the third entry into the params diff       */
/* (Task 88). Canvas multi-select opens the same dialog for a pair you */
/* can see; the picker answers the case where the twin run lives off-  */
/* screen or in another workspace: pick it from a list instead.        */
/* ------------------------------------------------------------------ */
...
function SiblingDiffChip({ current, sibling }: { current: JobDTO; sibling: JobDTO }) {
  const s = summarizeParamDiff([asDiffJob(current), asDiffJob(sibling)]);
  if (s.total === 0 || s.allSame) {
    return (
      <span
        data-testid="inspector-sibling-diff"
        data-diff-kind="same"
...
function SiblingComparePicker({ job }: { job: JobDTO }) {
  const jobs = useWorkflowStore((s) => s.jobs);
  ...
  // the inspector can hop to a different job at any moment (breadcrumb,
  // lineage) — a stale compareWith from the previous job must never open
  React.useEffect(() => {
    setCompareWith(null);
    setOpen(false);
  }, [job.id]);
  ...
  return (
    <>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            data-testid="inspector-compare-button"
        ...
        <PopoverContent
          ...
          data-testid="inspector-compare-popover"
        >
            ...
            {siblings.map((sib) => {
              ...
              return (
                <button
                  key={sib.id}
                  type="button"
                  data-testid="inspector-compare-option"
                ...
                >
                  ...
                  <SiblingDiffChip current={job} sibling={sib} />
                </button>
              );
            })}
          </div>
      ...
      <ParamsDiffDialog
        // the inspected job anchors LEFT (teal) — the picker is "this job vs
        // that one", so the baseline is where you already are
