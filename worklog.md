...
---
Task ID: 160
Agent: main (cron window 2026-09-13 04:31:02 +08:00, trace …202609130436)
Task: 例行七条——开局核对（摘要第六次过时，实际尾部 Task 159）+ QA 判稳 → 选题（左栏 tab 会话位置：三部曲收官）→ 实现 + t160 探针（×3）→ 受影响面回归（两起探针世界敏感性修复一起探针时序修复）→ 全矩阵 95 套 → 交接闭环。本轮交付「会话位置三部曲」的最后一块：你闭眼时左栏开着哪个面板，睁眼时原样开着——Catalog | Workspaces 的选择活过 reload

Work Log:
- 【开局核对】worklog 尾部实际为 Task 159（75dcde7 == origin/main，树净）——摘要停在 Task 153，连续第六次过时；BUILD_ID fzDUtxCYPZJQ18WrYphrZ 匹配；冷启动 2s READY + 三件套（qa00/qa63/t159）全绿 → 稳定
- 【选题】Task 159 交接候选全真机/让位/否决在案 → 重估：digest failed 行内 Retry 考古后发现 toast 已有 View+Retry 双门（store.ts:1003 announceRetryAction），行内重复入口会稀释 t146「summary not a door」边界，让位；store 侦察发现 page.tsx:467 `<Tabs defaultValue="catalog">`——左栏 tab 每次 reload 被强制扔回 Catalog。合同审定：不是过滤器（切换不藏东西，另一面板一键可达）、不是镜头（不改语义），是纯位置——「你在用哪个面板」，与 activeWorkspaceId（站哪块画布）/selectedId（看哪张卡）同族。三部曲缺的第三块，定案
- 【实现·page.tsx 单文件三处】① LEFT_RAIL_TAB_KEY="cryoflow.leftRailTab.v1" + LEFT_RAIL_TABS 白名单 + hydrateLeftRailTab（裸串 trim + **白名单信任门**——job id 无格式可白名单所以信任门必须是现实本身，tab 值是有限集合所以信任门就是白名单本身，两代合同互为镜像）+ persistLeftRailTab（window 守卫 + try/catch）② **boot effect 一次性 hydrate 而非 useState 惰性初值**——aside 是 SSR 首渲染就存在的（t156 gallery 有「数据未到 UI 未生」护体，aside 没有），eager hydrate 会跟 server HTML 打架触发 hydration mismatch；useState 从诚实默认起步，effect 在 hydration 完成后落 seed ③ 受控 Tabs（value + onValueChange **包漏斗**——Radix 把鼠标点击与键盘方向键汇入同一 handler，无编程式切换路径，一个漏斗一次包装零漂移；persist-then-set）。boot 零写（hydrate 只读，effect 永不 persist，t157-F 负 oracle 持续成立）
- 【t160 探针·39 断言 ×3 全绿】S 世界快照（tab 不需要种子）；X 十三 oracle（key/SSR 守卫/白名单/includes 门/corrupt 兜底/persist 形态/**唯一写路径=1 定义+1 调用**/boot effect 三连（存在+落 seed+永不写）/受控 Tabs 无 defaultValue/persist-then-set 顺序/Task 160 文档锚）；B 新世界 boot 零写+诚实默认 Catalog+真实点击 Workspaces→storage 同步回声+tabpanel 活跃；C **CORE reload 后 tab 仍 Workspaces**；D 双向门（"catalog" 是字符串修正非删除）+ **键盘 ArrowRight 过同一漏斗**（Radix roving focus 实测 storage 同步回声）；E 种子诚实三连（"nonsense" 诚实落 Catalog 且 boot 不改写陈旧 seed / ""=诚实的未知 / "  workspaces  " trim 过门）；F fresh page 零 cryoflow key；G 屏摄；Z 严格 console+roster。**疤痕**：页面有两组 role=tab（header 的 Dashboard/Workflow 视图切换 + 左栏的 Catalog/Workspaces），裸 [role=tab] 选择器命中 header——断言必须 scope 到 aside；Workspaces trigger 的 textContent 膨胀（"Workspaces3"），匹配用 startsWith 不用等值；/tmp 下跑 debug 脚本解析不到 playwright（t159 疤痕重演：脚本必须在项目目录内）
- 【回归·三起探针修复皆非 feature 所致】① **t157 B3 稳定失败**：诊断取证 ALPHA 卡 DOM y=-248.5 在视口外——canvas 是 transform-panned 世界，playwright scrollIntoView 够不到 pan 出视口的卡（世界 pan 记忆被前轮探针改动，t157 假设「种子卡在屏内」就破）。移植 t88 配方：relocateCardToFreeBand（max-y+480 构造性空带）+ panUntilVisible —— 世界敏感性第五演 ② **qa58 同族失败**：SEL_JOB 同样可能在 pan 外，同款移植（relocateSeedToFreeBand + panSeedUntilVisible）——第六演 ③ **qa68 flash 断言失败**：两层剥洋葱——第一层 volume 数据消失（qa59 PASS 后的 SEED_CLEAN 连 orthovol 一起清：--clean 删 engine-state entry → outputs 列表空 → dims undefined；qa58-seeder 重种 + qa67-seed-volume 恢复）；第二层 flash 仍失败 → playwright 高频采样诊断（120ms 间隔 2.6s）**实测 flash 在 t≈2ms 与 t≈1062ms 都亮着、readout z 64/64**——产品与 echo 链完好，是 agent-browser eval 冷启动（CLI 进程 ~0.3-1s）错过 650ms flash 窗口。修法：单次采样改窗口内循环采样（20 次 ×120ms，任一见 cyan 即过——断言不变量，不赌单帧）
- 【全矩阵 95 套（t160 auto-include 位 #81）10 块 0 失败】qa59（块 1-10）/t143（块 61-70）各单套抖动一次、单跑全绿；qa68 修复后绿；41-50 块 600s 工具上限截断于 t127，t128 补跑绿（telemetry 可回溯进度——Task 154 交付的 verdict 列全程服务）；t152 193s 仍最慢；块峰 207MB（阈值 1200MB）零重启；qa59 收尾复核再绿
- 【收尾】worklog + commit + push + 环境清理

Stage Summary:
- 「会话位置三部曲收官」：selectedId（t157：看哪张卡）→ activeWorkspaceId（t159：站哪块画布）→ leftRailTab（本轮：用哪个面板）——三个分量合起来才是完整的「你闭眼时在哪」。家庭成员逐个按合同审：tab 是位置不是过滤器（切换不藏东西），进入 stay-put 家族免费且正当；keptOnly/notedOnly 是过滤器继续 ephemeral 的判决不受影响
- 「信任门的两种形态互为镜像」：无格式的 id（job）信任门必须是现实本身（解析到画布成员谓词）；有限集合的值（tab）信任门就是白名单本身（命名一个本构建真的渲染的 tab）。判据不是偏好是值的拓扑：值的空间越大越开放，信任门越要落在现实上；值的空间封闭，白名单就是现实。t160 的注释把这个镜像写进了源码，下一个想持久化有限集合状态的人直接抄
- 「SSR 渲染面不能惰性 hydrate」：t156 的 useState 惰性初值成语有前提（「数据未到 UI 未生」——gallery 在 client effect 后才挂载）；aside 是 SSR 首渲染就出的，同款成语会触发 hydration mismatch。boot effect 一次性 hydrate 是 SSR 渲染面的正确成语——诚实默认起步，hydration 后落 seed，与数据到达同帧。两个成语按「组件是否出现在 SSR HTML 里」选型，不是按喜好
...
---
## Task 175 (2026-09-14, cron 03:16 窗口 trace …202609140325)

**主题：affordance 可读性轮（触屏工艺第六轮，Task 173/174 交接三候选合流）——diag 四粒沙实锤并全修：①destructive toast 的墨（X 红300 双主题 2.49/1.50:1，title 2.94:1）；②真根因：`text-destructive-foreground` 自 shadcn v4 迁移起从未生成（@theme 漏映射 --color-destructive-foreground，t174 静默生成课的姊妹篇，四组件族靠继承意外活着）；③swipe 回声硬编码黑在暗卡上 Δ0 像素；④grabber 永静态。外加探针诚实失败牵出第五粒：canvas fit 的 ZOOM_MIN 钳位 + 居中裁剪把首载最左列藏到栏下（t170 C5b 十四轮隐藏预设的坍塌）。t175 探针 32 断言 ×3 全绿；全矩阵 110 套两遍（修复后诚实全重跑）零失败零抖动（qa75 五连绿）。**

- 【开局】worklog 尾部 = Task 174/0df107f（cron Task 13 文本第廿次过时）。HEAD==origin/main 树净、BUILD_ID JTjldJtyGVqnqu7yiaJRd、PORT FREE；冷启动 32s 即 200；世界 26 正典（16c/8i/1f/1r）；三件套 qa63/qa00/t174 全绿判稳；agent-browser 巡检 console 零错。
- 【选题侦查】交接三候选（destructive 对比度/grabber 动效/回声暗色）合流「affordance 可读性」。diag-affordance.mjs（合成 drop 触发 destructive——collectDroppedFiles 的 .files fallback 是代码注释明写的 synthetic-drop 合同路径）四项实锤：①X 红300 on 红600 2.49:1（light）/1.50:1（dark——dark token 是更亮的红400 oklch 0.704，红墨配更亮红底）；②**title computed lab(17.6) 近黑**——`text-destructive-foreground` 类在构建 CSS 零规则、@theme 只映射了 --color-destructive 没映射 --color-destructive-foreground，destructive 表面的文字自迁移起靠继承 --foreground 意外活着（light 近黑 on 红 = 2.94:1）；③回声 rgba(0,0,0,0.16) 硬编码黑，暗卡上像素 Δ≈0；④grabber animationName "none" 永静态。
- 【实现·七处】①@theme 补 `--color-destructive-foreground` 映射（一行根修——toast variant/ToastAction hover/四个删除按钮的墨全部复活）；②.dark --destructive 0.704→0.577 与 light 统一（红一族两主题同色，白墨 4.57:1）；③X text-red-300→red-200（3.28:1 双主题）；④ToastDescription `group-[.destructive]:opacity-100`（90% 混红底 4.3→4.57 过 4.5 小字线）；⑤回声 `color-mix(in oklab, var(--foreground) 16%, transparent)`（墨色随主题：light 近黑同原观感、dark 近白 Δ17 像素可见）；⑥grabber `.swipe-grabber-hint` 一次性动效（@keyframes 7px 朝退场方向 nudge + 2px 回弹，0.55s 延迟等入场落定，1 次迭代 fill both，prefers-reduced-motion 静默——mount 即播、每次打开恰好一箭；keyframes 骑 `transform` 与 Tailwind v4 原生 `translate:` 属性合成——独立通道，居中 -50% 不被覆写，构建 CSS 实证）；⑦t173 X9 oracle 同步新合同。
- 【探针三课（第二层真相三连）】①**computed 色要经浏览器自己的转换器**：Chromium 把 oklch token 解析成 lab(...) 输出，人肉把 Lab 分量当 RGB 算出垃圾对比度（DARK title「330:1」假象）；1×1 canvas 光栅化是唯一诚实 sRGB 路径。②**像素采样要问像素的坐标系**：iPhone-13 截图是 dpr=3 物理像素，CSS 坐标采样采在画布上——完美 Δ0.0 是两块相同画布补丁不是不可见回声；80px 浅拖时可见性还随 progress 线性缩放（0.205×0.16≈3.3% 有效 alpha），像素合同要乘 devicePixelRatio 且在有意义深度测量。③**CSSOM 扫描要递归 @layer**（Tailwind v4 规则全住 layer/media 容器里）——首版 D1 零命中是遍历深度假阴性。
- 【t175 探针·32 断言 ×3 全绿】S2 基线；X10 源码 oracle（映射行/教义注记/dark token 统一无 0.704 残留/red-200 无 red-300 残留/opacity-100/color-mix 无 rgba 残留（注记豁免——合同钉 `background: "linear-gradient(to right, rgba` 的缺席）/hint 类/keyframes 0.55s+1+both/reduced-motion 块/合成+不抢焦点双门注记）；M14 移动相 iPhone-13 上下文（drop 路径真派发 destructive、LIGHT 3.28+4.57 实测、desc opacity 1、DARK 翻转同数、nudge 活体 name/iter=1/fill=both、mid-nudge 偏移 7.5px 且 36 宽居中保持（丢 -50% 会读 ~25px）、rest 0.50px、reduced-motion none、深拖 transform+progress 0.56=cueOp、28% 退场、暗回声 Δ17.0≥12、console 合同卫生 /log 404 配对）；D4 桌面相（CSSOM 递归证明 `.text-destructive-foreground{color:var(--destructive-foreground)}` 真实生成、双主题同数——一合同两主题两视口、console 零）；Z 名册恒等。
- 【插曲·t170 位链失败 → canvas fit papercut（第五粒沙）】首轮矩阵块9 t170#91 挂 C5b/C6 → 单跑复现（非抖动）→ 逐层侦查：`[data-job]` 首匹配是 55×24 微卡（fit 后 27% 缩放）→ 点击超时日志揭示 sticky 目录头拦截 → elementFromPoint 链 + frameBounds 数学复盘：fit 宽度约束 992/4646=0.2135 被 **ZOOM_MIN=0.25 钳位** → 居中裁剪把内容均分裁掉两边 → 世界长宽增长（minX=150、右缘 x+CARD_W≈4700）终于踩线，最左列（QA Post 300/320/385、Import Movies 1）落在 x=215——栏(0..288)下且被 section overflow-hidden 裁剪，不可见不可点。**非本轮回归**（toast/globals/swipe 与画布几何零交集），是世界增长压垮 t170 十四轮的隐藏预设（「首 recorded 候选卡在屏」）。产品修复：frameBounds 在钳位咬住时**锚定原点角**（`max(48, center)`——96px 预算拆对称 48px pad；容得下时居中仍胜出，容不下时原点侧恒可达、裁剪只落远端且 pan 可达）。验证：x=215→**336**（=288+48 精确命中）、t170 单跑 73 断言全绿。
- 【回归 + 全矩阵两遍】修复前矩阵跑至块9 揭伤（109 套中 t170 一败）；fit 是共享几何 → 修复重建后**诚实全重跑 110 套 10 块前台**：110/110 零失败零抖动，t170 位 #91 本位 PASS（原失败链位链复现全绿）；qa75 位 #16 五连绿；块峰 204MB（qa81）远低 1200MB 阈值；t152 190s 一致最慢带。触屏簇 t171(35)/t172(37)/t173(39)/t174(29)/t175(32) + qa63/qa00 七连绿。
- 【世界收尾】矩阵诚实消耗 Live 行 → qa60-seed-fsc 重建 → **26 jobs（16c/8i/1f/1r 正典构成精确复现）**；卫生八审全零 + domain-sweep 0/0/0；agent-browser 巡检 console 零错（27% 缩放下最左列全部入画——修复的可见证据）；定妆照四张（toast light/dark 右下裁片：白墨 on 同一红、暗色 X 清晰；暗色 nudge Sheet；回声带像素证据）。
- 【收尾】worklog + commit + push + 环境清理（杀 server 先 ss 查真实 PID、agent-browser close --all、diag 脚本归档 scripts/ 留档）。

Stage Summary:
- 「类名在场≠规则存在」的 @theme 变奏：`text-destructive-foreground` 死了整个 v4 迁移期，destructive 表面的文字靠继承意外活着——没有人发现，因为红底上的近黑墨「看起来还行」。一行 @theme 映射复活四组件族。CSS 合同的最后一环不在源码、不在类名，在构建产物的规则表里；t172 钉解析像素、t174 钉生成规则，t175 把这一课从 arbitrary value 推进到 theme mapping 层
- 「测量的第一原则：让被测系统自己做转换」：oklch token 的 computed 值是 lab(...)，人肉色彩数学把 Lab 分量当 RGB 得出「330:1」的假绿和「1.63:1」的假红——直到改用 1×1 canvas 光栅化（浏览器唯一自己做的 sRGB 转换）数字才落地。对比度合同断言的是光栅化后的像素关系，不是色彩空间的理论值
- 「完美的一致性本身就是假信号」：暗色回声像素 Δ0.0 精确为零——不是「不可见」而是「根本没采到」（dpr=3 的物理像素坐标系里用 CSS 坐标点了两下画布）。可疑的干净和可疑的脏一样要追凶；采样协议的第一问是「我点的坐标活在谁的坐标系里」
- 「探针的诚实失败是世界预设的 X 光」：t170 C5b 十四轮活在「首候选卡在屏」的隐藏预设上，世界每轮长宽各长一点，ZOOM_MIN 钳位终于咬住居中裁剪——探针不是脆，是它量的用户路径（点开首条 completed 记录看 argv）真的碎了：首载三卡不可见。位链教义的第一步（复现失败场景）把一例「矩阵抖动」假象拆成了真产品修复；修的是 fit 的合同（容不下时锚哪），不是探针的阈值
- 「钳位处是合同最容易烂尾的地方」：contain 语义 + zoom floor 的组合必须回答「容不下时锚哪」——居中裁剪把损失平均分给两边，等于把最常用的原点角也藏起来。锚定原点让裁剪只落在可 pan 达的远端；max(48, center) 一行同时保住「容得下时居中」的旧观感
- 世界卫生观察账本（verdict 列）：全矩阵 110/110 零失败零抖动（qa75 五连绿）；0 环境性失败；八审全零；正典构成精确复现
- 遗留（下轮候选）：destructive token 统一后其余 destructive 面（按钮/徽章/alert 边框）的暗色一致性抽查（理论同色，未逐面拍照）；grabber nudge 的真机手感（agent-browser 合成事件盲区依旧，须真机）；320px 以下 280px 折叠屏折内态（Task 173 遗留）；undo 手感参数（真机）；runner wall-time 剖面（t152 190s 一致带，让位）；EMPIAR 真数据回归（重，让位）；用户真机项（三级阶梯+长按+swipe+grabber+toast 手势+本轮 nudge 可真机验收）

---

## Task 176 (2026-09-14, cron 05:31 窗口 trace …202609140531)

**主题：280px 折叠屏折内态极限验收（Task 173/175 交接候选③）——census 五粒沙全修：①幽灵 X（头条）：Task 171 的 hit-slop 编辑混入的 `relative` 在 TW4 工具类级联中压过 `absolute`（.relative 排在 .absolute 后），sheet Close 计算 position:relative，top-4/right-4 变成 in-flow 盒上的相对偏移，X 停在内容流底部 y=653（653 视口外）——主退场 affordance 自 171 轮起在所有移动宽度上可聚焦、slop 养肥（44×44 位置无关几何，t172 的 resolved-pixel 探针因此全绿）却完全不可见，280 census 实证 rect [-15..264]×[653..669]；②palette sheet w-72(288) > 280 视口（side="left" 无 <sm 钳制）；③inspector tab 栏 ~357px 触发器在 96vw 对话框内被自身 overflow-hidden 裁剪（280 下 Files 不可达、320 同伤、390 也削 23px）；④log 控制台工具栏 Tail/Full/Follow 簇 + 搜索 ~380px 越界不可达 + severity legend 行溢出 33px；⑤边线 hover 命中走廊（隐形 16px pointerEvents:stroke）在触屏上偷走跨越卡面的 tap（25% 地板缩放下布线密集，这也是本轮早期 pan 轮盘赌 tap 失败的真根因）+ minimap 192px 框在 280 视口盖住 69% 宽且坐进 FAB 区。t176 探针 60 断言 ×3 全绿；全矩阵 111 套三遍（含一次陈旧构建产物 #418 的诚实全重跑）零失败零抖动。**

- 【开局】worklog 尾部 = Task 175/640def9（cron Task 13 文本第廿一次过时）。HEAD==origin/main 树净、BUILD_ID zurtyRVCbPkWs138JXSNN、PORT FREE；冷启动 1s 即 200；三件套 qa63/qa00/t175 全绿判稳；favorites 长按/Sheet swipe/grabber/320px 极限验收核实均为 162–175 轮已兑现，交接候选收窄为 Task 175 余项（destructive 面抽查/nudge 真机/280px/undo 真机），选题 ③ 280px。
- 【census 两遍】diag280.mjs（视口相对）：canvas/inspector/toast/dashboard 全净、FIT 锚点 280 下 9 卡全见（Task 175 原点角合同下探成功）、job sheet tabs 需 260 ≤ 280 意外健康；唯一命中 palette sheet 288>280。diag280b.mjs（**dialog 相对坐标系**——overflow-hidden 下的隐形裁剪比溢出更危险）：inspector tab 栏 over=88px（四 tab 全族伤）、cmdk/shortcuts 净。diag280c-e 破 X 之谜：静止态 sheet 零裁剪 vs b-pass 的 over=15 是**左缘越界被我的 over 标向算反**，顺藤摸出 Close computed position:relative 的幽灵态（diag280e 实证 pos:relative + rect y=653）。
- 【实现·八处】①sheet.tsx Close 删一个词 `relative`（absolute 自身就是 ::before 的 containing block，slop 无需第二 position）；②palette SheetContent 加 max-w-full（twMerge 与 sm:max-w-xs 共存：≥sm 不咬，280 钳到 280）；③inspector tab 栏 max-sm 压缩：icons `max-sm:hidden`（aria-hidden 装饰，标签达意）×4、触发器 `max-sm:gap-1 max-sm:px-1.5`、容器 px-5→px-3、TabsList `max-w-full`——桌面 ≥sm 保持 Task 114 原样，280 下 4 tab 224 ≤ 274 全可达；④log 控制台工具栏 `flex-wrap` + 动作簇 `max-sm:flex-wrap`（Tail/Full/Follow/搜索/换行全可达）；⑤severity legend 行 `max-sm:flex-wrap`（error 章 33px 越界）；⑥palette 头部行 `max-lg:pr-9`（<lg palette 必为 sheet，复活的 X 原来正好坐在计数徽章上——"3X" 叠字实证后让位）；⑦**边线 hover 走廊 mouse-only**：edges-layer 命中 path 从 inline `pointerEvents:"stroke"` 改 globals 类 `.edge-hit-path`（桌面保 pointer-events 关键字 stroke——无 TW 工具类发它）+ `@media (hover:none){ pointer-events:none }`（项目自有输入模态惯用法，Task 174 toast 先例）——触屏 tap 恒达卡面，桌面 hover 走廊保留；⑧minimap `hidden lg:block`（192px×170 框在 280 视口 = 69% 宽 × 底 1/4，默认常开坐进 FAB 区；桌面仪器 <lg 让位）+ dock 的 minimap-toggle `max-lg:hidden`（无死控件）；t105 的 store 字面量合同与桌面行为原样。
- 【探针教义·三课】①**定位的确定性**：随机 pan 循环是 harness 自己的抖动发生器——placeCard 改从 `[data-canvas="workspace"]` 的实时矩阵 + job 世界坐标**算出拖拽增量**，卡落在断言需要的地方（t176 M 相零 tap 抖动）；早期「tap 打不开 inspector」读数全是定位竞态不是产品伤——确定性定位后触摸 tap 每发命中（含原失败点位复测）。②**「伪元素合同量几何不量位置」**：t172 的 44×44 hit-slop 断言在幽灵 X 上照样全绿——::before 的 resolved 尺寸与按钮的 position 无关；三个触屏工艺轮拍了 header 没拍角落。③**「级联是类表的第五列」**：`relative` 与 `absolute` 同时在类表里时，TW4 的产出顺序（.relative 后）决定生死——tabs.tsx 的同名模式是有意设计（flow 元素给 ::before 当 containing block），sheet Close 是纯误伤。
- 【插曲·React #418 陈旧构建产物】迟来修复轮的构建与 palette 编辑同消息并发，产物疑似混态：全矩阵 v2 跑到 chunk 7 时 t132/t133/t134 三连挂 Z1（Minified React error #418 text 水合失配）。判别三步：单跑复现 ✓（非抖动）→ 撤 palette 注释重建 t133 复绿 → 注释恢复+干净重建三连全绿 → **注释无辜，是构建吃到了编辑中途的混态树**。教训固化为时序铁律：src 编辑与 npm run build 永不同消息并发；矩阵失败先查 BUILD 时间戳与源码 mtime 的先后。
- 【t176 探针】60 断言 ×3 全绿：S5 基线（idle/completed Import 各一在册）；X15 oracle（双 position 缺席/44 slop 存活/palette max-w-full/inspector px-3+max-w-full+max-sm 压缩+icons 4/4/log 工具栏 wrap/legend wrap/palette pr-9/corridor 类+globals media 块/minimap lg 门控/toggle 同隐）；M 相 280×653（FIT 锚点 5 卡、幽灵 X 复活四连：absolute+on-screen+44 slop+**点击真的关掉 sheet**、sheet census 0 裁剪×2、tabs 270≤280、dock 279、inputs ≥40、palette 280≤280+X 在屏+徽章 231≤X 247、corridor computed none、minimap 隐藏、inspector bar 224≤274+icons none+Files 可达激活+四 tab census 全零、toast 248@x16、cmdk/shortcuts 净、console 合同卫生）；D 相 1440（bar 带 icons fits、icons 活、corridor stroke、minimap block、tablet 700 band palette 288=sm:max-w-xs 与 max-w-full 共存、console 零）；Z 名册恒等。
- 【回归 + 全矩阵三遍】v1：chunk1 qa61 抖（单跑复绿→原位复跑 8/8）；chunk12 t173 挂 M4「Expert trigger absent」=**探针隐藏世界预设现形**（pan 取任意 idle 卡，只有正典 idle Import 必有 Expert 选项——十四轮运气）→ 按 t176 定位教义修探针选择器（idle Import 优先，断言合同不动）→ 原位复跑绿；chunk13 t87 抖（单跑复绿→原位绿）。v2（迟来修复后）：chunk7 #418 三连挂→判别陈旧构建→干净重建。**v3 终局：111/111 零失败零抖动**（#418 三人组原位绿、t176 slot#97 in-slot 49s、qa75 位#16 六连绿）；chunk 峰 207MB（t158）远低 1200MB 阈值；t152 191s 一致最慢带。
- 【世界收尾】矩阵诚实消耗 → qa60-seed-fsc 重建 → **26 jobs（16c/8i/1f/1r 正典构成精确复现）**；卫生八审全零 + domain-sweep 0/0/0；agent-browser 巡检 console 零错；定妆照四张（palette 280：徽章 36 让开 X 全族可见；sheet 280：grabber+复活的 X+tabs+dock；inspector 280：四 tab 无 icon 全 fits；agent 巡检截图留档 scripts/）。
- 【收尾】worklog + commit + push + 环境清理（杀 server 先 ss 查真实 PID、agent-browser close --all、diag280 a-o + tour/shots 归档 scripts/ 留档）。

Stage Summary:
- 「级联是类表的第五列」：类名在场≠规则生效，规则生效≠你想要的那条赢——`relative` 和 `absolute` 同表时 TW4 的产出顺序（.relative 后）裁定。tabs.tsx 同款是有意设计（flow 元素给 ::before 当 containing block），sheet Close 是误伤；一个词的差，主退场 affordance 幽灵了五轮。审计合同要测 computed position，不是类名存在
- 「伪元素合同量几何不量位置」：t172 的 44×44 slop 断言在 off-screen 幽灵上全绿——::before 的 resolved 尺寸与宿主 position 无关。几何探针（宽高/像素比）与位置探针（viewport 内/computed position）是两种病两套药；「量过」不等于「量对地方」
- 「dialog 相对坐标是 overflow-hidden 时代的普查面」：视口相对普查在 overflow-hidden 容器时代会漏掉整族伤（inspector tab 栏/log 工具栏/legend 在视口普查里全净）；第二遍普查改问「谁被自己的容器裁了」——被裁不可达比溢出可滚更恶性，因为连 scrollbar 的求救信号都没有
- 「探针的定位必须是计算出来的，不是逛出来的」：随机 pan 是 harness 抖动的自产自销（t173 十四轮的「Expert trigger 在场」预设、本轮的「tap 失灵」假读数全是它）；从实时矩阵+世界坐标算增量，卡落在断言需要的位置，tap 每发命中——世界预设的 X 光片要用确定性的光源拍
- 「hover 的走廊在触屏上是抢劫犯」：桌面 hover 命中区（16px 隐形走廊）在 (hover:none) 世界里只剩成本没有收益——跨越卡面即偷 tap。输入模态变体（hover-none:）是项目自有惯用法，pointer-events 关键字值走 globals 类
...
- 【实现·九处】①②page×2/canvas×1/job-card×1 四处 confirm 全部归一到家族串 `h-10 bg-destructive text-destructive-foreground hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40`（h-10 是 M3e 实测 36px 违反项目自家 ≥40 舒适线后补的）；③button.tsx/badge.tsx 变体去 text-white、去 dark:/60（token 红双主题全α）；④失败卡图标补 dark:text-rose-400；⑤inspector 身份行 `max-sm:flex-wrap`（长名标题独占整行、徽章让位换行，短名照旧同行；≥sm 原样）+ h2 title={job.name} 悬停还原；⑥job-panel/project-panel/project-dashboard 三处 B 家族确认钮补齐 ring 位并入家族串。
...
## Task 179 (2026-09-14, cron 14:01 窗口 trace …202609141406)

**主题：功能轮——the pipeline earns its replay script。per-job 命令三档合同（Task 170）早就在，但流水线级产物从未存在：JSON 导出是 CryoFlow 内部格式（graph+params），报告是 per-run——builder 用户的终点（拿走一条可运行的流水线）没有 answer。本轮全链新功能：GET /api/projects/[id]/pipeline-script（Kahn 拓扑 + createdAt 平局 → 确定性；每步沿用同一三档合同：native 注释块 / buildArgv 真命令 / canonical template+原因；replay 语义按状态：done+idle 活命令、run/wait/fail 注释+原因；set -eu；整个 artifact 一次时钟读数）+ PipelineScriptDialog（sbatch 家族范式：六色 census 徽章 + pre + Copy/Download，preview before download）+ palette 新行。附带擒获一个真实产品伤：**整个导出家族在移动端不可达**（palette 触发钮 hidden md:block——Ctrl+K 需键盘，触屏无任何入口）。t179 探针 70 断言 ×3 全绿；全矩阵 114 套完成（108 首跑绿 + chunk9 三连位链闭环）。**

- 【开局】worklog 尾部 = Task 178/aee8d45 全绿闭环（cron Task 13 文本第廿四次过时）。HEAD==origin/main 树净、BUILD_ID XJF23T5ZyS_DvK0SSjk_w、PORT FREE；冷启动 3s；世界 26 正典（16c/8i/1f/1r）；三件套 qa00(#1)/qa63(#7)/t178(#99) 全绿判稳（113 套时点位）；agent-browser 巡检 console 零错。交接候选核实：grabber nudge/undo 真机盲区、minimap 过度工程关、EMPIAR/wall-time 让位——触屏工艺已连六轮，按惯例⑤转功能面。侦查 qa_lib 种子文件与 API 消费差集时确认 class2d 类分布已有（class-distribution-chart），图表家族 22 组件 + 19 API 路由已密；palette 有 Export PNG/JSON 唯独无「流水线导出」——选它。
- 【实现·五处】①`src/lib/relion/pipeline-script.ts`：buildPipelineScript（project 全量 jobs+edges → link 塌缩（mirror 让位 oldest original）→ Kahn 拓扑（createdAt 平局→确定性输出，两导出仅差时间戳）→ 每步三档：ENGINE_NATIVE_TYPES.has→注释块（模板 description 代言 executor 内部 stage）；CLI+RELION 缺失→template 注释+re-export 提示；CLI+inputs missing/wait→[wait]+原因；CLI+argv ok→done/idle 活命令 shellJoin——shellJoin 自 command route 迁入（一个引用规则、两个消费者，preview 与 script 不可能漂移）；failed→[fail]+run.err 提示、running→[run]+永不竞速 live run）。只读合同（command route 的项目级推广：无 mkdir/spawn/写状态）。②route：薄壳 404+委托。③command route 删本地 shellJoin 改 import（t170 X8 八标识符不动——shellJoin 不在其名单）。④`pipeline-script-dialog.tsx`：RELION 缺失 amber 横幅、六色徽章（done teal/ready emerald/running amber/waiting slate/failed rose/native violet，零值徽章隐藏）、pre max-h-55vh、Copy（clipboard 降级不杀 Download）、Download blob+slug 文件名。⑤palette 行「Export pipeline as shell script · dependency-ordered relion replay · all workspaces」+ close-then-open 握手（一屏一 modal）。
- 【第二层真相·三连】①**「两次时钟读数从未一致」**：首轮确定性断言挂——header 内嵌时间戳与 generatedAt 字段是两个 new Date()，毫秒级互异；「两导出仅差时间戳」的合同必须整个 artifact 共享一次读数。②**「oracle 匹配自己的注释就不是 oracle」的自我戏仿**：X10 数 new Date() 得 2——第二个匹配是我自己的教义注释（「two new Date() calls never agree」）；剥注释后数（t166 方法应用于探针自身）。③**「B15 断言错字符串」**：step 注记是「race a live run」我断言 header 的「moving target」——单跑诊断过、探针必挂，确定性假非瞬时抖动；读数要来自被断言的那一层，不是同 dialect 的另一层。
- 【产品伤·移动端 palette 不可达】M 相挂：390 视口下触发钮不存在——header 包 `hidden md:block`，Ctrl+K 需键盘：**palette 及整个导出家族（PNG/JSON/脚本）在触屏零入口**（统一缺口非惯例）。诊断先行：diag-t179-header.mjs 实测 280-768 七宽度余量——280 折内 ~24px slack 放不下 40px 触发钮（首版 sm:block 修法无效，sm=640px！390 仍在其下）；终修=触发钮恒显（<sm 图标态 max-sm:px-1.5→26px）+ 腾位自最低价值 chrome（GitHub 链接钮 max-sm:hidden——命令面旁边的装饰品）；七宽度复测 over=0 全零，280 折内保持 Task 176 预算。「仪器降级让位功能」惯用法第二次应用（minimap<lg 先例）。
- 【t179 探针·70 断言 ×3 全绿】S5 基线（roster 26/native+CLI+failed+running 目标）；X13 oracle（共享表 import 无 body 拷贝（t166）/buildArgv+topo+shellJoin 导出/command route 消费共享 shellJoin 无本地拷贝/只读合同剥注释扫描 lib+route/prisma 零 mutation/set -eu/六 marker 方言/一次时钟读/palette 行/dialog 五 pin/fetch URL）；B20 live 合同（census 三方恒等 steps==stats==roster 26、marker census 求和、记录行=marker 行且唯一、native 注释块+workdir 行、环境免疫 CLI 档（RELION 缺失分支：19 步全注释模板+各自原因）、fail+run 步语义、**16 条真实边全 parent-first**（steps.line 拓扑断言）、确定性两导出仅差时间戳、generatedAt 与 header 同一读数、只读 dir-listing 恒等、404）；M7/D8（palette→dialog 全流程 390/1440、dialog 视口内、pre 载真脚本、stats 徽章、Download slug 文件名实证、Escape 关、console 双零）；Z4（roster 恒等、无 workdir 垃圾、无意外 5xx）。
- 【回归】t170(#91) PASS（shellJoin 迁移零伤——X8 名单不含它）；t176(#97)/t177(#98) PASS（header 触发钮改动零伤）；t111(#37) PASS（palette 新行零伤——合同只钉 copy 组行数）。
- 【矩阵·114 套 10 块】t179 auto-include（114=113+1）。块 1-8/10 首跑全绿；块 9 t177/t178/t179 三连挂 S 基线（running job undefined）=**矩阵诚实消耗 Live 行**（Task 177 同款第二次遭遇：修世界不修探针）→ qa60-seed-fsc 重建 → 三套单跑复绿（53/36/70 断言）→ **chunk 9 原位复跑全绿**位链闭环。qa75 位 #16 八连绿延续；块峰 210MB（t160）远低 1200MB 阈值；t152 191s 一致最慢带。
- 【世界收尾】qa60 重建 → **26 jobs（16c/8i/1f/1r 正典构成精确复现）**；卫生八审全零 + domain-sweep 0/0/0；agent-browser 巡检：palette 行渲染、dialog 全流程走通、console 零错；定妆照两张（pipeline-dialog.png：诚实横幅+六色徽章+脚本 pre；patrol.png）归档 scripts/shots-t179/。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、agent-browser close --all、diag-t179-header.mjs 归档 scripts/ 留档）。

Stage Summary:
- 「per-step 合同的复利与它的项目级空缺」：Task 170 的三档合同是 per-job 的——但 builder 用户的终点是整条流水线。route 级能力天然可组合，组合层（拓扑序+replay 语义+确定性）才是新功能；共享表（COMMAND_TEMPLATES/ENGINE_NATIVE_TYPES/buildArgv）让组合层只用 ~150 行
- 「两次时钟读数从未一致」：任何「仅差时间戳」的确定性合同，时间戳必须一次读数整个 artifact 共享——两个 new Date() 的毫秒差就是合同谎言
- 「移动端入口是功能存在性的另一半」：palette 功能齐全≠移动端可用——触发钮 hidden md:block 让 PNG/JSON/脚本三导出在触屏零入口，多年无人发现因为桌面键盘 muscle memory。加入口前先量（280 折内 slack 24px 放不下 40px），腾位自最低价值 chrome（装饰性外链让位命令面）；断点教训：sm=640px 不是 390——「修法无效」先查断点数学
- 「探针读数要来自被断言的那一层」：B15 断言 step 注记（「race a live run」）时抄了 header legend 的词（「moving target」）——同 dialect 不同层的字符串。单跑诊断 pass+探针 fail 的分歧=探针在断言自己想象的世界
- 「矩阵失败先分世界消耗与探针伤」第二度验证：chunk 9 三连挂 S 基线 = Live 行被诚实跑完；qa60 重建+原位复跑位链闭环（Task 177 教义第二次应用，流程成熟）
- 世界卫生观察账本（verdict 列）：全矩阵 114 套完成（108 首跑绿；t177/t178/t179 世界消耗位链闭环）；qa75 八连绿延续；0 环境性抖动；八审全零；正典构成精确复现
- 遗留（下轮候选）：script 的 RELION-present 分支在本沙箱无法实测（detectRelion false 恒真——安装 RELION 或 mock 后 B12 的 live-commands 分支需一次真实踩线）；pipeline-script 的 cycle appendix 分支零覆盖（canvas 防环，防御诚实但无世界能踩到）；dialog 深色主题定妆照未拍（token 全走语义变量理论安全）；grabber nudge/undo 手感参数（真机，盲区依旧）；runner wall-time 剖面（t152 191s 一致带，让位）；EMPIAR 真数据回归（重，让位）；用户真机项（三级阶梯+长按+swipe+grabber+toast+nudge+fold+palette 触屏入口+本轮 dialog 可真机验收）

## Task 180 (2026-09-14, cron 15:46 窗口 trace …202609141546)

**主题：一致性轮——所有把边落进世界的门共享同一条环规则（the import door learns the canvas's one rule）。审计推翻 Task 179 交接里的一个「既记载事实」：pipeline-script 的 cycle appendix 并非「无世界能踩到」——POST /api/workflow-import、custom-template save、custom-template apply 三扇批量门都跳过了环检查，手编 JSON 即可把 A→B→A 的依赖死环送进世界（canvas 与 POST /api/edges 都防环，唯独批量门不防——「pathref 与 star 路由包含策略不一致」的环规则变奏）。修复=一个共享纯检测器（src/lib/graph-cycle.ts，迭代式三色 DFS）+ 五处消费（client 预校验/import parseBody/template 共享校验器/apply 重查/edges API 迁移收编），环被拒时点名（"Edges form a cycle: MC Alpha → MC Beta → MC Alpha"）。t180 探针 41 断言 ×3 全绿；附带审计判决：autoStartPendingDownstream/particles BFS 有 seen 集免疫、hpc simulate 有 10000 步 guard 无悬挂、pipeline-template 的 TEMPLATE_EDGES 是出厂常量不设防（判决注释入码）。**

- 【开局】worklog 尾部 = Task 179/546799e 全绿闭环（cron Task 13 文本第廿五次过时）。HEAD==origin/main 树净、BUILD_ID kAn1BXKIXT_n7AGP4jCa4、PORT FREE；冷启动 11s；三件套 qa63/qa00/t179 全绿判稳；agent-browser 巡检 console 零错。交接候选核实：Duplicate 已存在（右键菜单/批量工具栏/⌘D 全有——侦查避免重复造轮）、job notes（Task 83）、activity feed、Gantt 时间线（analytics 第三视图）均已在；detectRelion 深入引擎 launch 路径（1283/1460/3712）伪造 RELION 存在会改变引擎行为——RELION-present 分支维持沙箱受限判决。
- 【选题·审计定性】转向 Task 179 交接候选②（cycle appendix 零覆盖）时发现记载有误：rg cycle/acyclic/DAG 在 workflow-import 与 template-io 全零命中——**import 门用 persistPortEdge 直写（route.ts:239），环守卫只住在 edges 路由的本地 DFS 里**。全遍历器审计八处：autoStartPendingDownstream ✓（seen 集合 dispatch.ts:241）、particles ancestry BFS ✓（seen 集合）、pipeline-script topoOrder ✓（appendix）、hpc simulateQueue ✓（while guard<10000 有界无悬挂）、GET jobs projectLinks ✓（无拓扑遍历）、edges API ✓（本地 DFS）、workflow-import ✗、custom-template save+apply ✗。workspace DELETE 是搬移非删除（jobs 移回默认 workspace）——探针清理必须走 DELETE /api/jobs/[id]（边级联）。
...
- 「记载的「不可达」要复核门的数量」：Task 179 写下「cycle appendix 无世界能踩到」时只数了 canvas 和 edges API 两扇门；第三扇（import）和第四、五扇（template save/apply）从未被清点。防御分支的可达性论证是全称命题——漏一扇门整个论证作废；审计要列全门清单再下判决
...
---
## Task 181 (2026-09-14, cron 17:31 窗口 trace …202609141731，终稿补于 18:31 窗口 trace …202609141831)

**主题：灾难恢复轮 + the percent that lied（标度合同轮）。开局即遇本会话最重环境事故：18:31 窗口接手时沙箱已回滚到 Task 172 快照——HEAD 从 4bd2a17 退到 33cd444，本轮前段（17:31 窗口）已修好未提交的 4200% bug 修复随未跟踪产物一同蒸发。恢复分两路：git 侧 `reset --hard origin/main`（远端保有全史，Tasks 173-180 代码瞬回）+ 世界侧全新重建（db/.next/data 全灭——`ensureProject` 冷启动 3-job 基座 → `restore-gallery.py` 活体实例 → `qa60-seed-fsc` FSC 家族 → 新写 `restore-canonical26.py` 补齐回滚吃掉的 5 名常住卡 + QA Bench 工作区 → **正典 26（16c/8i/1f/1r）精确复现**）。产品伤：Dashboard Recent activity feed 的进度标签把 0–100 wire 值当 0–1 fraction 再 ×100——running 行渲染「4200%」+ bar 恒满格（17:31 窗口巡检截图实锤）。修复=fetch 边界一处归一化（toFractionFrame），五处 ×100 渲染位点原样保留（它们没错，错的是没人翻译 wire）。t181 探针 23 断言 ×3 全绿——**钉的就是真 wire**（原 probe 败因：mock 42% fraction 喂进去，假绿上路）。附带修复两枚世界敏感探针：t175 M13 echo 像素改同像素双态对比（Δ11→18.6 真值）、t177 band 几何换私人舞台 + 多跳 placer + 矩形包含守卫（53 断言 ×3 全绿）。全矩阵 116 套（t181 auto-include）10 块前台：108 首跑绿；chunk 8 的 t175（世界敏感）与 chunk 9 的 t177-t181 五连（Live 行诚实消耗，第四度）修复后位内复跑全绿。**

- 【开局·环境事故】worklog 尾部=Task 180/4bd2a17（17:31 窗口已核实，三件套全绿、巡检零错、发现 4200% 并修毕未提交）。18:31 窗口接手：HEAD=33cd444（Task 172）、`toFractionFrame` 零命中、t180-e2e.mjs 不存在、scripts 192 个（Task 172 时代）——**沙箱整棵回滚**。`git fetch` 后 origin/main=4bd2a17 完好 → `reset --hard` 代码全回（worklog 5113 行含 Task 180、t180 探针回归、prisma schema 与 172 零漂移）。未跟踪产物全灭：db/custom.db、.next/、data/ 全空。
- 【世界重建链】`prisma db push` 重建库 → 冷启动遇 **data/ 缺失半种子事故**（`registerProject` 写 data/projects.json ENOENT——项目行建了 3 jobs 没落；ensureProject 的 count>0 短路让重放不再补）→ 清空 db 重触发 → 3-job 基座 ✓ → `restore-gallery.py`（21 jobs 16 edges + qa58/50/51/52/53/60/67 fixture 链）→ `restore-canonical26.py`（新写：QA Bench 工作区 + MotionCorr 002 failed（run.err 说真话 exitCode 1）+ QA Set Rate/QA Polish Staging/QA Class Staging/QA Refine Staging idle——名字尽力读取 Task 180 定妆照，三个 Staging 名诚实标注为重建推测）→ 26 精确复现。next build 首败（@next/swc-linux-x64-gnu 缺失——回滚也清了平台二进制）→ `npm i @next/swc-linux-x64-gnu@16.1.1 --no-save` → 构建过。
- 【4200% 修复（重放+探针）】/api/activity/recent 返回 DB 原值 42（0–100 标度，与画布 chip/inspector/JobRow 同源）；feed 内机全是 fraction 语义（bar ×100 clamp、label/aria ×100、sparkline pct ×100、absorb 重启阈 0.05、趋势阈 0.005）。修=RecentJob 接口文档钉明双标度 + `toFractionFrame`（wire→internal 唯一翻译点）+ 两个 fetch 帧（mount+4s poll）都过它；route.ts 补 SCALE CONTRACT 注释。t181-e2e.mjs：S3 wire∈[0,100]、X8 源面 oracle（合同注释/唯一翻译点/两帧都过/×100 位点存活数/JobRow 不动）、B9 live（**feed 无四位百分数**（4200% 金丝雀）、aria==bar==label==round(wire)、tap chip ≤100、poll 帧复验、console 零）、Z3 roster 恒等。×3 全绿。教训入骨：**mock 的合同不是 wire 的合同**——原 probe 用 mock 42% fraction 自测自绿，真 wire 上线即 4200%。
- 【t175 M13 修复】swipe echo 暗色像素断言 Δ11.0<12 恒挂（Task 180 矩阵还绿）——旧采样比的是同帧两个不同像素（边缘 x+4 vs 腹地 x+70），阈值被「腹地恰好画了什么」隐性标定；重建世界换了 pickFirst 的卡，标定失效。修=**同像素双态对比**（拖中 echo-on vs 拖回 echo-off 同一坐标）——内容抵消只剩 echo 本体，真值 Δ18.6，阈值回归原设计 12。教训：**像素阈值要么同像素双态，要么注定被世界冻结期掩盖**。
- 【t177 修复（最深的一口）】band 选择断言挂三连（M3a 0/2、M3b title null、D11 4/2）。三层真相层层剥：①dbg 实锤 B 的放置拖拽 mousedown 落在已就位的 A 卡内→抓走 A（互擄）；②placeCard 只平移视图不改世界坐标——格网邻卡刚性随行，平移推不净 band 区（"others inside band 2" 三连）；③修复三件套：**私人舞台**（API PATCH 把配对迁到格网正南空旷带 y=1600、间距 400 世界 px，Z 相恢复原坐标——世界读回与先前一字不差）+ **多跳 placer**（每跳取候选的可达分量（构造性保界）、单调收敛，远舞台几跳可达；修掉增量当绝对坐标用的低级错）+ **guard1 换矩形包含判据**（中心距判据对刚性对永不满足——65px 间距重坐时 B 随行）。53 断言 ×3 全绿（M3a/D11 双 2/2）。教训：**几何探针的舞台必须私有**——「恰好没邻居」不是合同，是运气。
- 【矩阵】116 套 10 块前台逐块：chunks 1-7/10 全绿首跑；chunk 8 的 t175（M13）修复后位内复绿；chunk 9 t177/t178/t179/t180/t181 五连挂 S 基线=Live 行诚实消耗（第四度验证）→ 清 5 个探针残留 + qa60 重建 + 五套单跑复绿（t177 修毕 53 断言）→ 位内 97-108 复跑全绿。qa75 位 #16 第十连绿；块峰 208MB 远低 1200MB；t152 191s 一致最慢带。插曲：qa45 首跑挂=agent-ctx/ 目录随回滚蒸发（mkdir 重跑即绿——环境性非伤）。
- 【世界收尾】26（16c/8i/1f/1r 正典构成精确复现，Live 行 42% 存活）；坐标污染清算（我手动 PATCH 3200,2400 的调试残留 + 舞台卡还原→Import Movies 1 (1600,160)/QA MotionCorr (2380,160)）；泄漏工作区 QA WS Breathe/QA WS Pos Two（chunk 9 崩溃套件壳）删除 → Main+QA Bench 双区；卫生八审全零 + domain-sweep 0/0/0；agent-browser 巡检 console 零错；定妆照两张（t181-patrol-feed.png：feed 行 42% 真值 + 三张 Staging 卡就位；t181-patrol-canvas.png：正典 26 全家福）归档 scripts/。
- 【收尾】worklog 终稿（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、agent-browser close --all）。18:31 窗口全程接手（trace …202609141831，Task 13 文本第廿七次过时）。

Stage Summary:
- 「mock 的合同不是 wire 的合同」：t181 的全部存在理由。probe 喂 mock 42% fraction 时全绿，真 wire 0–100 一接就 4200%——探针的价值在它钉住「线上真实的那一跳」，合成数据只能钉「组件数学自洽」。凡断言带量纲的合同（百分比/字节/时区/标度），探针必须从真实数据源取值，mock 只配测纯函数
- 「回滚灾备的三层各自独立」：git 层 origin/main 全史可 reset；构建层 swc 二进制要按平台补装；世界层 db/data 是不可版本化的活物——qa_lib + restore-gallery + qa60 + 新 restore-canonical26 四级种子链把它从零推回正典。教训的新变种：**ensureProject 的幂等短路（count>0 return null）让「半种子」不可自愈**——部分失败的种子比没有种子更难恢复，幂等钩子要在写路径上而非读路径上
- 「像素阈值要么同像素双态，要么注定被世界冻结期掩盖」：t175 M13 的 Δ12 是对「某个旧世界某张卡腹地画了什么」的标定，世界一动就现形。同像素双态（echo-on vs echo-off 同坐标）让内容抵消——阈值从此只对 echo 本体负责。同理 t177 的 band 区「恰好没邻卡」也是冻结期运气——私人舞台让运气变合同
- 「平移不改世界坐标」是画布探针的第一性事实：placeCard 全家都是视图平移，配对与其邻卡刚性同行——想隔离就动世界坐标（API PATCH+恢复），别指望视图。增量当绝对坐标用的一行错让 8 跳全在原地打转——dbg 埋点（cur/d/best 逐跳打印）十行内定位，盲猜三次不如埋点一次
...
## Task 186 (2026-09-14, cron 23:16 窗口 trace …202609142316)

**主题：调度器收编轮——the simulate route earns its face, and the oracle convicts the engine。Task 185 交接首选项兑现：POST /api/hpc/simulate 自 HPC 层落地起完整在位（Slurm 形状的调度器：priority-FIFO + afterok 依赖边 + GPU 池 first-fit + array 分片 + 实测时长按 profile speedup 缩放），却零 UI 消费。Task 186 把 HpcQueueSim 挂进 sbatch dialog：四参数运行表单（GPUs/nodes/array conc./GPU speedup，min/max 写合同）、KPI 带（makespan/GPU util/avg wait/GPU-hours）、甘特图（一 job 一行、array 分片为同行分段、type 着色三元组+fallback、分钟标尺+四分位细线、slurmId·label·资源·节点 的 title tooltip）、pending episodes 琥珀脚注、clamped 脚注。「Server view shown」教义第二度：每次 Run 后输入框从 RESPONSE echo 重同步——UI 显示服务器留的，从不显示发出去的。探针 B12 的调度器 oracle（bar 不得先于依赖结束 + GPU 池任意时刻不超额）定罪两个产品级 bug：①simulateQueue 依赖门控用 doneAt.has(d)（键存在=已放置）而非完成时间比较——MotionCorr 在 0.2m 开跑而其 import 跑到 1.0m；②speedup 旋钮对模型分钟全聋——8 个 idle 主链 GPU job（refine3d 47.5m）走 strategy.minutes 原值，×500 拧了等于没拧（makespan 48m==48m）。两处根修 + t186 47 断言 ×3 全绿。**

- 【开局】worklog 尾部=Task 185/f2f2aa0（cron「Task 13」文本第卅二次过时）。树净、上窗口杀净 server → start-prod.sh 冷启动 3s；三件套 qa00/qa63/t181 全绿判稳；巡检 console 零错。
- 【实现·两文件】①新建 hpc-queue-sim.tsx（~370 行）：section 挂 sbatch dialog 底部（默认收起，Run 展开并请求）；NumField×4 携合同钳位范围；gpusPerNode 预填 GPU 池（gpusTouched 守卫——用户键入的值赢过预填，Task 185 手法回声）；KPI 带 tabular-nums；甘特 rows useMemo 按 key 聚合首次出现序（=调度序），分段左缘/宽度按 makespan 百分比、0.6% 可见下限、宽度≥8% 才画 GPU 徽章；TYPE_COLOR 十族字面量三元组（import sky/motioncorr cyan/ctf teal/extract violet/select amber/class2d fuchsia/class3d indigo/refine3d rose/postprocess emerald/maskcreate green）+ FALLBACK；line-clamp-2 的 note 带 title 全文。②sbatch dialog 挂载点传 profiles.find(p=>p.id===profileId)?.gpusPerNode。jobType() 注册表供 label（不露裸 key）。
- 【探针 t186·47 断言 ×3 全绿】S5（roster 26、idle 门卡、无 hpc-profiles.json、simulate 可达 89 bars）+ X10 源 oracle（POST 消费、jobType 注册表 label、四 KPI 格、echo 重同步、clamp 脚注、预填守卫、百分比定位+可见下限、PENDING 计数、dialog 挂载 oracle、十族色板）+ B14 活线（89 bars≥20、events≥bars、makespan>0、util∈(0,1]、默认簇形状 echo 8/4/8/×25、敌意 999/999/-3/100000 钳到 64/16/1/×500、×500 击败 ×1、**依赖语义 oracle：16 wire 边 0 afterok 违规**、GPU 池峰值 8≤8、makespan==末 bar 端）+ D13 UI 全环（门卡→HPC→面板→Run→KPI→预填 4 echo→甘特 89 段→tooltip→999 echo 64+clamp 脚注→1 GPU 拉长 makespan 39m>30m→Escape）+ Z5（roster 恒等、registry 从未写、零 5xx/404、console 净）。
- 【第二层真相·双定罪】①**doneAt 把「已放置」冒充「已完成」**：runnable 门控 `doneAt.has(d)` 只查键——而 doneAt 在放置当时就写入未来 end 值，上游「被排上」的同一瞬间下游就「可跑」，时钟被别的 0.2m CPU job 推进后立即抢池。B12 首跑 5 违规（MotionCorr 0.2<import 1.0、CTF 8.4<MotionCorr 16.2、QA Select 12.4<13.2、QA Post 13.2<Refine3D 13.4 ×2）——violation 模式（起点<依赖终点且 GPU 空闲即抢）是比计数更硬的证词。修复=`(doneAt.get(d) ?? Infinity) <= t`（afterok 等的是值不是名字）+ inFlight 修复恒空病（placed 且完成值>t，否则提前 break 风险）。②**speedup 聋的 fallback**：路由只缩放实测分支；无 run state 的 job 落 strategy.minutes 原值——恰恰是用户最关心的 idle 晚期主链 GPU job。修复=模型 GPU 分钟同公式缩放 `(strategy.minutes * 10) / speedup`（floor 0.2），note 文案同步。两修后 makespan(pool4)=30m（util 100%——依赖串行下池被真正榨满）。
- 【探针自身的三课】①**正则要读到类型注解之后**：`const TYPE_COLOR = \{` 匹配不到 `const TYPE_COLOR: Record<string, string> = {`——oracle 静默落空转成 fallback 分支的 "{}" 假证词；改 `[^=]*=`。②**格式化值的空格是捕获类的墙**："1h 03m" 的空格截断 `[\dhm.]+` 捕获成 "1h"→NaN；改读 aria-label（cell 自带 `Makespan 1h 03m`）+ 容忍 h/m 双组的 parseMin——a11y 标签再次比 textContent 可靠。③**门控后的控件不存在**：面板收起时输入框不在 DOM，D3 首跑读到空串；预填的证明改走 echo 往返——Run 后框里的 "4" 就是 POST body 携带预填的指纹（默认态是 8，echo=4 ⇒ 预填说过话）。
- 【法证工具的静默失败】B12 取证时 sqlite3 CLI 根本不存在（exit 127 被 `|| true` 吞掉→「DB Edge rows: 0」假 ground truth，差点把调查引向「边表为空」的死胡同）——Prisma client 才是真值读者（15 DB 边齐全）。教训：法证脚本的每条 ground truth 都要报告自己的读取是否成功。
- 【回归】qa00 GREEN、qa63 SMOKE GREEN（console 0）、qa58 ALL PHASES GREEN、t184 33 断言全绿（HPC 合同面无回归）、t185 55 断言全绿（registry 管理面无回归）。矩阵 119→120 套（t186 auto-include，槽位 #106；显式清单+尾部 auto-include glob 的双保险无需手改）。
- 【世界收尾】正典 26（16c/8i/1f/1r、Live 行存活）；探针全程只读（Z 相证明 registry 文件从未写、roster 恒等）；定妆照两张（t186-panel-collapsed.png、t186-panel-gantt.png——KPI 带 30m/100%/6.0m/2.3h、8 pending episodes 琥珀、甘特 type 着色+GPU 徽章+分钟标尺）归档 scripts/shots-t186/；agent-browser 巡检 Dashboard+Workflow 双视图 console 零错后 close --all。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID）。

Stage Summary:
- 「afterok 等的是值，不是名字」：调度器把 doneAt 的键存在当完成——上游「被排上」的瞬间下游就自由了，并行世界靠一个 Map.has 的语义滑动伪造。依赖是时间点不是成员资格；`?? Infinity <= t` 三字符的修正把 squeue 语义还给甘特
- 「旋钮必须拧得动它声称控制的机器」：speedup 只缩放实测时长，模型 fallback 全聋——而 idle 的晚期 GPU job 恰恰是用户打开模拟器想问的那批。旋钮的承诺关于硬件等级，不关于哪些 job 恰好在沙箱里跑过；fallback 与实测必须听同一个旋钮
- 「调度器 oracle 让图画变成合同」：bar 不先于依赖结束 + 池任意时刻不超额——两条不变量把 Gantt 从插图升格为可断言的调度证明。没有 B12，那两个 bug 会带着「看起来很忙」的甘特活到真集群头上
- 「violation 的模式比计数更硬」：5 条违规全都指向同一形状（起点<依赖终点、GPU 空闲即抢）——模式指认机制，计数只报规模；而「DB rows: 0」的假 ground truth（sqlite3 不存在被 ||true 吞掉）提醒：法证读者必须先自证读到了
...
## Task 197 (2026-09-15, cron 05:31 窗口 trace …202609150531)

**主题：报告会师终局——the families meet under one cover。三位报告父亲各据仪器深处（t194 sweep、t195/t196 地图 QC）；t197 让他们在单一会话文档下重逢：`@/lib/qc-report.ts`（家族的唯一家——第二消费者到来，bundle 卫生与教义同一理由）+ `session-report-dialog.tsx`（react-markdown+remark-gfm 渲染的文档页：pipeline glance 活读 store、地图 QC 免开 viewer 走同一 map-profile API 实测、sweep 附件逐字绑定）+ header 的 FileText 门 + store 的 lastSweep 切片 + 打印例外契约（body[data-report-print] 时只有报告上纸——Markdown→PDF 全线贯通）。顺手把 t195 宣告的「无时间戳」教义回迁到 sweep（Generated 行移除）。t197 50 断言 ×3 全绿；回归 14 套全绿（11 处前浪 oracle 同步——单次晋级的最大前浪账单）；矩阵 130→131。**

- 【开局】worklog 尾部=Task 196/9164407（cron「Task 13」文本第卌三次过时；内嵌摘要「04:46 零动作」第 10 度过时——04:46 窗口实际完成了 Task 196）。树净、冷启动（reaper 本窗口格外凶：裸 setsid 两次被清，最终 watchdog 配方稳住）；三件套全绿判稳；agent-browser 巡检双视图 console 双零错。
- 【实现·七刀】①新 src/lib/qc-report.ts：resampleByFraction/pearson/agreementVerdict/pctAt/pairwiseAgreement/ReportOverlay/buildProfileReport 从 molstar 原文搬家，buildSweepReport/SweepProfile/SweepRow/fmtMin/sweepReportFilename 从 hpc 原文搬家（Generated 时间戳行移除——同一场比赛永远产出同一字节），buildSessionReport 缚文之父（家族输出逐字嵌入、绝不 parse、零时间戳、诚实空态三连：无任务/测量中/测量失败各自有词）；②session-report-dialog.tsx：walk（completed × updatedAt 降序 × outputs 3D 体积 × cap 8）→ map-profile ×3 → buildProfileReport，门三扇（Copy/Download/Print），回执双世界，载体 data-md 与 data-report-doc 同驻常驻；③header：FileText 门 + nextDynamic 分包（外壳不为文档渲染器付费）；④store：lastSweep 切片，compare() 收尾整槽写入（profiles 拉取失败=没开赛，不打扰上一场）；⑤globals.css：打印例外 + .report-doc 排版；⑥molstar/hpc 删本地双胞胎改 import；⑦remark-gfm 落地（表格是家族的母语）。
- 【探针 t197·50 断言 ×3 全绿】S3（seeder + note 幂等 touch 定胜负——Prisma @updatedAt 是 walk 的决胜局）+ W3（探针用 API 独立重演同一条 walk 规则、自己算 peak/pearson/pairwise）+ X14（ONE home、无本地双胞胎、缚文之父零 parse、三位 builder 零 new Date、store 写入、载体同居、三门实名、打印例外在册、gfm 在册）+ D20 活线（门→文档开卷→glance==wire→地图节说 walk 胜者==峰值==r→pairwise 在册→sweep 诚实空态→打印旗上 body→print 媒体下只有报告上墨（getClientRects 绘制性 oracle）→回执双世界→关门拔旗→真 race 后附件逐字绑定（胜者加粗==真档名、margin 句、无一人被静默丢掉）→地图节与附件同页共存）+ Z3 只读。
- 【第二层真相·四】①**定妆照是唯一看得见全栈的观察者**：打印例外在屏幕上全绿（computed 样式全部清白、DOM 断言无偏移），真纸却每行都从单词中间开始——shots 的 printToPDF 逮到真 product bug。「computed display 会说谎（祖先 display:none 时自身仍报 block），getClientRects 才是绘制性的 oracle；而 PDF 是绘制的终审」。②**Tailwind v4 的居中走原生 translate: 属性（t175 判例第 2 度），且 minifier 会吞掉复位**：`transform:none` 复位不了 `translate:-50%`；更险的是 lightningcss 把 `translate:none` 和 `translate:0 0` 都「等价折叠」进 transform 简写——个体属性与 transform 是独立属性，折叠是假等价，Tailwind 的 utility 照常生效。复位只能骑 minifier 无法求值的 var()：`translate: var(--report-reset-tx, 0 0) !important`。「优化器的等价假设是跨规则级联的赌注；把赌注变成它读不懂的东西」。③**walk 需要确定性决胜局**：updatedAt 是「本会话最近动过的图」的诚实语义，但探针不能赌它——note 幂等 touch 把决胜局攥在手里，同时探针用 API 独立重演同一条 walk 规则：「断言是两个实现的一场对话，不是一个希望」。④**shots 必须与探针走同一条世界开门链（t196 教义第 3 度）**：无 S 相的 shots 拍出了 regression 残渣的胜利者（单图、无对比表）；世界态的继承不看故事大纲。
- 【回归·14 套全绿（终版构建重跑）】t194(33，X1/X8——builder 搬家)/t195(40，X1/X4/X8/X9/X12 五处同步)/t196(29，X1/X2/X3/X5 四处同步)/t191(40，X7 计数跨家族 2+1)/t190(29)/t189(42)/qa67(27)/qa58(GREEN)/qa42(exit 0) + 三件套 qa00/qa63/t181。**「前浪 oracle 跟后浪源码走」单窗 11 处——一次晋级的最大前浪账单**；D 相活线零摆动（搬家不动行为）。
- 【世界收尾】正典 26 精确保持（Z 相只读证明）；定妆照四件归档 scripts/shots-t197/：t197-report-2x.png（紫门文档页全景）+ t197-report-1x.png + sample-session-report.md（载体真字节 2058B）+ **t197-report.pdf（Print 门的亲手产物 103KB、两页纸质文档——Markdown→PDF 的出生证明，也是它自己逮到那个 print bug 的物证）**；t197-shots.mjs 留档（自带 S 相）。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「会师不是合并，是装订」：session 报告不重述家族的话——它把家族的字节逐字装订（bind, never re-translate）。pipeline glance 活读、地图 QC 实测、sweep 附件逐字——一份文档里三种时间性（live/measure/bound）各自诚实，只有一位父亲（buildSessionReport）负责扉页、空态与「不猜」的措辞
- 「第二消费者的到来重新给每个锚点定价」：两位 builder 晋级 lib 的那天，11 处前浪 oracle 当窗清偿——知识住一份的教义不免费，但双胞胎分叉的代价从来只会在第三位消费者手里结算。lib 是唯一同时满足教义与 bundle 卫生的家（import 组件文件=把 Mol* 拖进文档的分包）
- 「打印例外必须 principled」：Task 70 的契约是「dialog 在纸上让位」；会话报告是唯一反例——它不是页面上的 chrome，它就是页面。body[data-report-print] 把例外钉在「报告开着」这一件事上，关门拔旗；paper masthead（Task 65 的打印版报头）也随例外让位——例外再小也要清点它覆盖的一切
- 「minifier 的等价假设是跨规则级联的赌注」：transform:none ≠ translate:none——独立属性、独立生效。lightningcss 的折叠让复位静默蒸发（构建产物里连声明都不见了），只有 var() 的不透明性能让它闭嘴。构建管线的优化是另一层需要被探针覆盖的「实现」
...
## Task 200 (2026-09-15, cron 08:17 窗口 trace …202609150825)

**主题：一致赢得轮廓——the agreement earns a contour。t198 的四分带给背叛一个「带」（weakest quarter 是地址），t200 给它一条「线」：`rProfile` 把共享分数轴上的滑窗（n/8 平面，钳位 4..16）以窗心等距走 24 个站位，每站独立算 Pearson——画进每枚 weakest-quarter chip 内的 sparkline（data-r-profile，dashed r=0 线、负站位红色 shading、线色随 overlay），轮廓是视觉仪器所以**不进报告**（报告保留四分表——t193 导出教义的报告章）；墙上画线，纸上留数。本轮是**回收现场**：上一会话在 08:03–08:24 完成实现+探针初稿后耗尽上下文中断（树里留着未提交的两文件改动与探针、无 worklog 条目）；本窗冷启动判稳后接手，修出一个真 product bug（NaN 站位必须是洞）与探针两处自身缺陷后 ×3 全绿；回归 14 套全绿零 oracle 同步；矩阵 133→134。**

- 【开局·判态与回收】worklog 尾部=Task 199/3449be0（内嵌摘要「07:32 零动作」第 12 度过时——07:32 窗口实际完成了 Task 199 深色控件皮肤）。树**不净**：molstar-embed.tsx/qc-report.ts 有未提交改动 + t200-e2e.mjs 与三个 dbg 脚本在册——Task 200 in-flight。核实：构建（00:02:14）晚于两文件 mtime（23:59/00:01）且 chunk 含 data-r-profile——server 已服 t200 代码；探针初稿 25 断言、实现完整。冷启动（watchdog 配方）+ 三件套全绿 + 巡检双视图 console 双零错判稳后，接手中断的工作而非另起炉灶。
- 【实现·两文件（上会话完成）+ 本窗三刀】①qc-report.ts：rProfile（滑窗 w=min(16,max(4,round(n/8)))，RPROFILE_STEPS=24 个窗心等距站位 [round(k/23*(n-1))-⌊w/2⌋, +w)，每站独立 pearson，两边重采样到 max 网格——pairwiseAgreement 同一把尺；n<4 诚实返空）+ 注释立约「NaN 站位是诚实的洞，绝不是零；轮廓是视觉仪器，报告保留四分表」；②molstar-embed.tsx：local row 的每枚 chip 内嵌 svg sparkline（data-r-profile、role=img、dashed r=0、负站位 #f43f5e shading 30%、polyline 骑 overlay 色、vectorEffect non-scaling-stroke），chip 文本逐字节不动（t198 的 X5/D5/D11 oracle 原样成立）；**本窗三刀**：③「洞必须被画出来」——polyline 按连续有限站位切段（runs），NaN 站位处线**真正断开**：不是零、不是桥接、更不是非法的 "x,NaN" 字节（SVG 会在第一个非法点处整条丢弃 polyline 并抛 console error——半图平坦头部就是活的案例）；title/aria 同步教「gaps are flat windows」；④探针 D5/D7 重写为站位图比较（x 反解站位 k、有限站位逐位比 RAW r、NaN 站位必须缺席）；⑤D14 改轮询（世界态落袋是异步 PUT）。
- 【探针 t200·28 断言 ×3 全绿】S（seeder）+ W（wire 喂轮廓 oracle：独立重演滑窗+每站 pearson——half1 全 24 站有限；half2 头 3 站 NaN——它的头部 bins 逐字节相等）+ X7 源 oracle（ONE father、轮廓零报告调用点、sparkline 可寻址、dashed/负站位/y 映射在册、**洞承诺在册**（runs 切段源码断言 X4b）、chip 文本逐字节幸存、轮廓住 chip 内）+ D14 活线（重开后双轮廓齐上、half1 轮廓==oracle 逐位（24/24 有限零洞）、负站位 6 rects==oracle、**half2 的 3 个平坦站位是画中真Gap**、轮廓穿 overlay 色、UI 移除溶解 half2 轮廓不扰邻居、空集墙上无轮廓、**会话服务端空**（轮询到 wire 同意））+ Z2 只读（roster 26 恒等、console 零错）。
- 【第二层真相·四】①**绘画必须兑现文档的承诺**：库注释写「NaN 站位是诚实的洞」，但初版绘图把 NaN 直接拼进 points——SVG 在第一个非法字节处静默截断整条线还抛错：诚实的语义若画不出来，就只是善意的谎言。「画不出来的诚实不是诚实」。②**断言的量化管线要与被测量化对象说同一种方言**：旧 D5 对比「预舍入的 oracle r」vs「y 量化后再舍入的 de-typed r」——两步舍入在最坏情况下差 ~0.011，half1 的 k15（r≈-0.9653 正好骑在舍入边界上）假 FAIL；修法=与 RAW oracle r 直接比（容差 0.0075 只吸收 y 量化的 ±0.0011）。「先数清自己断言里舍了几次，再定容差」。③**观察者的显示层会伪造语法错误**：工具输出的渲染管线把源码字面量 `[hi` 吞成 `i`——sed/node JSON/python repr 三路「读到」不存在的 `binsi]`，grep 计数、布尔检查、Edit 工具却都说文件没病。「渲染出来的字面量不是字节；断言真相要打印布尔与码点，别信被渲染的文本」——t199「grep 行首撒谎」的显示层表亲。④**世界态的落袋是异步 PUT，不是点击的回执**：D14 在移除后 1.8s 读会话得到残员、一分钟后才空——固定睡眠是对 wire 的赌注，轮询到一致才是合同。「持久化的完成时刻在 wire 上，不在点击上」。
- 【回归·14 套全绿（重建后构建重跑，零 oracle 同步）】t198(34)/t196(29)/t195(40)/t193(29)/t191(40)/t190(29)/t189(42)/t192(14)/t197(50)/qa67(27)/qa58/qa42 + 三件套 qa00/qa63/t181（新构建复跑）。chip 文本的逐字节 oracle 天然免疫 SVG 嵌入（textContent 不含 svg），轮廓只住在墙上一处、报告未动——插入点再一次是对前浪断言的尊重。矩阵 133→134（t200 auto-include）。
- 【世界收尾】正典 26 精确保持（Z 相只读证明）；定妆照归档 scripts/shots-t200/：t200-panel-2x.png（双地形+双轮廓 chip+pairwise chip+四门的活照）+ **t200-chip-row-2x.png（chip 行特写——half1 青色轮廓在 Q3 背叛带有红 shading 深谷（r -0.99），half2 紫色轮廓以真 Gap 开场（3 个平坦站位无字节）——教义自证在案）**+ t200-wall-2x.png（探针活照）；dbg×4 临时脚本删除；t200-shots.mjs 留档（API-staged 世界+归还要齐全）。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「总分 r 是均值，四分带是地址，滑窗轮廓是地形」：t195 让分叉可数、t198 让它可定位（哪一带）、t200 让它可辨认（什么形状）——half1 的轮廓是「高原→悬崖→深谷→复原」，一个数（-0.94）和四个带（0.32/0.82/-0.94/0.68）都说不全的叙事，一条 56×10px 的线说完了。QC 仪器把「这张图不可信」收窄成「长这样的分叉」
- 「平带说 NaN，画布就得画洞」：语义层的诚实（NaN 不是零）要靠绘制层的诚实（真 Gap 不是桥接）来兑现，否则 SVG 的静默截断会把「这里没有形状」画成「从这里开始不存在」——洞的位置本身就是信息（half2 的头是平的，线就该从 13% 深度才起步）
- 「轮廓是视觉仪器，报告保留四分表」：不是所有数都该上纸——墙上的线负责形状的辨识度，纸上的表负责数字的可引用性；rProfile 只有一位父亲但只有一个消费者，`visual instruments don't enter reports` 写进源码注释当界碑（t193 的 CSV 教义在报告章的落地）
- 「两步舍入的断言要在 RAW 上会师」：期望值舍一次、观测量舍两次，最坏差 0.011 而容差只有 0.006——k15 的 -0.9653 骑在边界上就把真绿判成假红。断言的容差要只吸收「被测系统真实的量化」，不吸收自己断言管线发明出来的量化
- 「工具输出是渲染过的世界，不是世界」：`[hi` 被显示层吞成 `i` 的这一课和 t199 的「grep 行首撒谎」同宗——观察工具的每一层（压缩器、渲染器、转义器）都可能替你「修正」现实；布尔值、码点、计数是不会被渲染管线重写的证词。看到可疑字面量的第一反应是换个不可伪造的问法
- 世界卫生观察账本（verdict 列）：本轮无全矩阵（单仪器轮，183 判例=定向回归覆盖 viewer+导出面+会话报告 14 套）；t200 ×3 + 回归 14/14 全绿（重建后重跑）；正典 26 精确保持；矩阵 134 套在册
- 遗留（下轮候选）：竖排控件 hover 态的视觉验证（探针断言了 hover 规则在册，hover 瞬态的像素验证留待真机）；r(fraction) 下一里——把轮廓的滑窗 r 与真 FSC 的球壳采样差距做成报告里的一行诚实 caveat（现已在 shape-agreement 标注）；grabber nudge/undo 手感参数（真机盲区依旧）；script RELION-present 分支（沙箱受限判决维持）；EMPIAR 真数据回归（让位）；runner wall-time 剖面（让位）

## Task 201 (2026-09-15, cron 09:02 窗口 trace …202609150903)

**主题：affordance 的过堂日——the hover state's day in court。t199 断言了 hover 规则在册，但规则的「在册」从没被活体审问过——diag 一上场就挖出三桩陈案：① Mol\* 的可见堆栈 hover 全死（ink 被 Mol\* 自家的 `!important` 焦糖 `:hover` 规则压死、bg 被内联 `background:transparent` + 皮肤恒在的 `!important` base 钉死——悬停除了焦糖一闪什么也不发生）；② 键盘焦点环被 `outline-style:none` 静音（Tab 穿过堆栈对键盘用户完全不可见）；③ Reset Zoom 被 app 工具条的最后一枚按钮像素级掩埋（两条右锚定行在同一点相遇——它从未可点过，双房间同病）。t201 三刀落定：布局平移 `top:52px`（主题无关——布局 bug 不是主题 bug）、hover ink+bg 以 (0,6,1)+`!important` 反杀（`:not([disabled])` 保残障语义）、`:focus-visible` 焦点环 + `:active` 按压。侦察还挖出第四桩：**每次画布 hover 都向 console 刷 `TypeError: reading 'is'`**（bundler 残缺命名空间 × Mol\* 结构行为管线）——monkey-patch 尝试两次被下游打脸后改判：行为截肢（spec 只留体素查看器用得上的三条），抛错的观察者从此不被注册。t201 37 断言 ×3 全绿；回归 8 套全绿零 oracle 同步；矩阵 134→135。**

- 【开局】worklog 尾部=Task 200/564376e（上窗收尾恰在本窗打开后 49 秒，HEAD==origin/main、树净、PORT FREE、无残留——会话重叠未成害）。cron 引文的 Task 13 老债清点**全数过时**：#5 fs/browse 已有 same-origin+Host 钉、#7 chart 热路径已全部 `cachedFileCompute`、#8 particles 已批量化 BFS、#13 useMemo-localStorage 无匹配——recital 的账本比代码落后了十几个窗口。判稳照旧（watchdog 一次冷启动 + 三件套全绿 + 巡检双视图 console 双零错）。
- 【侦察·diag 先行】diag-t201-hover 三发现（上述①②③），并锁定赢家规则清单：Mol* 的 `.msp-btn-link-toggle-off:hover{color:#dc9c56 !important}` 与 toggle-on 孪生（`#ae5d04 !important`）——特异性战争在这一层是「对方也在用 ！important」；`top:10px` 来自 molstar.css 的普通规则 (0,1,0) 而非内联——双类 (0,3,0) 即可不可平局获胜。
- 【实现·两文件】①globals.css 追加 t201 块：`.msp-plugin .msp-viewport-controls.msp-viewport-controls{top:52px}`（主题无关、双类不可平局——工具条底缘 44px + 8px 呼吸）；hover `button[class]:not([disabled]):hover{color/background !important}`（(0,6,1) 压 Mol* 的 (0,3,0) !important——双方都举白旗时特异性是唯一裁决者）；`:active{transform:scale(0.96)}`（纯几何，无人争夺）；`:focus-visible{outline:2px solid var(--msp-accent) !important}`（对着不知住在哪个选择器里的静音者，不赌）。每处 ！important 带论证注释——例外要清点它覆盖的一切。②molstar-embed.tsx：spec 级行为截肢——`behaviors` 允许清单只留 `Camera.CameraAxisHelper / Camera.CameraControls / State.SnapshotControls`，HighlightLoci/SelectLoci/FocusLoci/Camera.FocusLoci/StructureFocusRepresentation/DefaultLociLabelProvider/CustomProps 全数出局。
- 【探针 t201·37 断言 ×3 全绿】S1（seeder）+ X9（源 oracle：布局规则无 `.dark` 前缀——掩埋是双房间病；hover 恰好两处 ！important 且饮 token；环在册；active 在册；规则体零 Mol\* hex——连敌人的颜色都只活在注释里；截肢 kept 恰三条；**无 monkey-patching 残留**——被撤掉的药的缺席本身也是断言）+ D17 活线（双房间 unbury 三连：可见、中心 elementFromPoint 命中自身、stack 顶清工具条底缘；hover ink==`--accent-foreground` 字节 [124,215,203]、bg==`--accent` 字节 [18,52,50]——焦糖 #dc9c56 阵亡；rest 恢复 secondary/muted-foreground；disabled 按钮过 pointer 不发光；`:active` matrix(0.96)；Tab 入栈 `:focus-visible` 命中 + 实线 2px 环==token 字节；**鼠标点击聚焦无环**——先点画布交还输入模态再点按钮；浅色房间 hover=原生焦糖 [220,156,86] 且 bg 透明、环静默——dark scope 的边界是形状）+ Z2（roster 26 恒等、**console 净——账本里记着一轮故意的画布扫掠+点击**，即护栏的激发步）。
- 【第二层真相·五】①**规则在册 ≠ 状态活体**：t199 断言了 hover 规则存在，悬停死了一直没人知道——探针断言语法，用户活在语义里；「样式轮的测试断言是对比度数字」的下一里是「状态轮的测试断言是触发后的字节」。②**!important 战争里特异性是唯一裁决者**：t197 教义（平局交给 chunk 顺序）与 t199 教义（button[class] 提到不可平局）的第 3 度——当对方也举 ！important，唯一的出路是更高的特异性 + 自己的 ！important；每处都要注释说清它反杀谁、浅色为何看不见。③**打地鼠不是修复**：normalize 护栏→isEmpty 抛→setFromLoci 抛——同一残缺命名空间病的多份拷贝在下游排队；而且第一版护栏自己差点把 init 搞挂（`reading 'bind'`）。真正的修复是退一步问「这条管线本该服务谁」——结构原子检视行为对体素图本无工作，把行为从注册面剪掉，抛错的观察者根本不被创建。**配置胜过手术，切除胜过包扎。**④**断言方法前先问方法住在哪个原型上**：guard 绑 `interactivity.normalizedLoci` 挂掉——该方法住 LociMarkManager 原型、由 lociHighlights/lociSelects 继承，interactivity 本体上根本没有；读类结构，不是读名字。⑤**探针自身的两条时序债**：开镜瞬态覆盖层（首跑 D-dark2 在 dialog 过渡的一拍上假红——重试窗胜过盲睡）；**输入模态残留**（Tab 走查把键盘模态留在会话里，点击后 Mol\* 重渲染的编程式恢复焦点继承它、`:focus-visible` 假阳——先点画布把模态交还鼠标再点按钮）。
- 【回归·8 套全绿（重建后构建重跑，零 oracle 同步）】三件套 qa00/qa63/t181 + t199(18)/t192(14)/t198(34)/t200(28)/t195(40)/qa67(27)。行为截肢未摇醒任何前浪——它们断言的是 app 自己的 UI 与数据，不是 Mol\* 的结构检视。矩阵 134→135（t201 auto-include）。五张旧定妆照被回归活线等价重摄（堆栈下移入画）。
- 【世界收尾】正典 26 精确保持（Z1）；定妆照归档 scripts/shots-t201/：**t201-toolbar-stack-2x.png（工具条行与完整堆栈同框——Reset Zoom 以第一枚 chip 重见天日）**+ **t201-hover-2x.png（Screenshot chip 点亮：青 tint 底+亮青 ink）**+ **t201-focus-ring-2x.png（Reset Zoom 佩戴 2px 青色实线环）**+ 深色全景 + 浅色全景（双房间掘出的证明）+ 探针活线 hover 特写；diag×3 临时脚本删除；t201-shots.mjs 留档（世界归还齐全）。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「规则的在册是语法，:hover 的命中是语义」：t199 让控件穿上了房间的调色板，t201 让它们的交互状态也活过来——第三方嵌入物的主题一致性的终审不是「静态长什么样」，而是「hover/focus/active 每一态的字节归谁」。五轮让位的债，利息是 hover 死了五轮无人知
- 「!important 战争的终审是特异性」：Mol\* 用 ！important 焦糖守卫 hover、用内联 transparent 守卫背景——特异性军备到天花板时的唯二武器是「更高特异性+!important」与「不进这条管线」。t201 两种都用了：hover 走第一条（状态要活），结构行为走第二条（管线该死）
- 「打地鼠不是修复，退一步问『这条管线本该服务谁』才是」：normalize 护栏被 isEmpty 打脸、isEmpty 会被 setFromLoci 打脸——bundler 残缺命名空间的拷贝散布多个 chunk，逐个包扎永远包不完；而抛错管线的宿主（结构检视行为）对体素查看器本无工作。**剪掉行为，观察者便不被注册；不被注册，便不会抛错。**配置面上的一行 filter，胜过运行时上的层层 try/catch——且零 monkey-patching 的缺席本身被探针断言（被撤掉的药不许复活）
- 「monkey-patch 的药自己也体检」：第一版护栏以 `interactivity.normalizedLoci.bind` 开场即抛 `reading 'bind'`——药差点成为病。形状检查（typeof）+ once-flag + 「不破 init」三重自保，是任何运行时补丁的入场费；但更好的答案往往在配置面等着你
- 「断言方法先问原型」：`normalizedLoci` 住在 LociMarkManager、由两个子管理器继承、本体上没有——名字一样不等于同一个方法。读第三方源码要读类结构图，不是读方法名；堆栈里的类名（LociHighlightManager.normalizedLoci）比文档里的类名诚实
- 世界卫生观察账本（verdict 列）：本轮无全矩阵（单 viewer 轮，183 判例=定向回归覆盖 viewer+主题+会话 8 套）；t201 ×3 + 回归 8/8 全绿（重建后重跑）；正典 26 精确保持；矩阵 135 套在册
- 遗留（下轮候选）：hover 色对的 WCAG 对比度数字进探针（--accent 底上的 --accent-foreground——本轮验了字节等价，数字审计留下一里）；竖排控件真机 hover 瞬态复核（headless 已活体过堂，真机可选）；grabber nudge/undo 手感参数（真机盲区依旧）；script RELION-present 分支（沙箱受限判决维持）；EMPIAR 真数据回归（让位）；runner wall-time 剖面（让位）

## Task 202 (2026-09-15, cron 10:17 窗口 trace …202609151017)

**主题：the address earns a door——地址可以点了。t198 给分叉一个地址（weakest quarter 是街号），t202 给街号装一扇门：weakest-quarter chip 从装饰 span 升格为真 button——点击→剖面平面跳到最弱带的分数中心（from/to 的 2dp 中心，景观条自己的方言），2D 正交面板骑同一 intent 事件同帧跟随，回执说人话（Plane moved to 63%…），键盘 Enter/Space 同门；hover/active/focus-visible 骑 app token。同时兑现 t201 交接的 WCAG 对比度数字普查：chip rest/hover + 景观条焦点环，双房间六个数字全部上断言——普查第一枪逮到既有债：景观条 focus ring 的 cyan-500/70 在浅色房间只有 1.70:1（<3:1 非文本底线），换实心 --ring token（light 3.07 / dark 6.66）。t202 43 断言 ×3 全绿；回归 14 套全绿（t198/t193 前浪探针的 Escape 闸门回迁）；顺手修 t201 遗留 tsc 错误 + 矩阵清单漂移（t200/t201 从未入册）。**

- 【开局】worklog 尾部=Task 201/c59461b（**内嵌摘要「Task 198/e490532」第 13 度过时**——摘要又落后两个窗口）。树净、HEAD==origin/main。冷启动遇 reaper：nohup watchdog 被收割一次、服务器死过一回，`setsid` 全脱离（PPID 1）才稳住。三件套全绿判稳；agent-browser 巡检双视图 console 双零错。
- 【实现·三文件五刀】① `@/lib/qc-report.ts`：LocalBand 增 `from`/`to` 分数字段（等数切法的**真实**分数界——机器地址住在人类词汇旁边，报告只饮 label/r **字节不变**，零前浪 oracle 同步；chip 跳深饮 from/to，绝不 parse 标签——parse-of-parse 教义在地址轴上的落地）。② molstar-embed：chip `<span>`→`<button type="button">`（onClick=applySliceIntent({pos:centre}) + flashProfileNote 回执；aria-label 教跳深；hover:bg-accent/hover:text-accent-foreground + active:scale-[0.97] + focus-visible:ring-2 ring-ring；textContent 逐字节不动——t198 D5/t200 轮廓 oracle 原样幸存）。③ 景观条 focus ring `ring-cyan-500/70`→实心 `ring-ring`（普查的既有债：浅色 1.70:1 → 3.07:1；t191 X4 源 oracle 同步一处——「前浪 oracle 跟后浪源码走」N 度）。④ t201 遗留 tsc 错误顺手修（Mol* 的 .d.ts 把 spec behaviors 型为裸 Behavior，运行时是 PluginBehavior——`as unknown as {behavior}` 双跳 cast 记录类型缝隙，运行时零变化）。⑤ run-matrix.sh 显式清单补 t200/t201/t202（**auto-include glob 只吃 t1[0-9][0-9]，t2xx 三窗从未入册**——矩阵总数诚实计数 130→133+）。
- 【探针 t202·43 断言 ×3 全绿】S3 seeder + W2（探针自己的 local oracle + 中心算术：half1 weakest Q3 (50–75%) r -0.94 → from 50% to 75% → centre 0.63 → strip 63%）+ X9 源 oracle（from/to 在册、button 非 span、跳深饮 applySliceIntent、**零标签手术**（w.label 只被引用不被 slice/match/parseInt）、回执实名、token 三态、**alpha ring 退役**、火花线仍住 button 内）+ D 相活线（D0 **先切暗色房**；Home→0% 后点 chip→strip==63 + 回执逐字节；End→100% 后 Enter 复跳 63%——键盘门在册；焦点环 `--tw-ring-color`==--ring 字节 2px；hover 字节==accent 对；census 六数字；报告 Local 表仍说 oracle 的地址）+ light 段（dialog 栈清空后切浅色、chip 在浅色房同说、census×3）+ Z3（roster 26 恒等、console 零错）。
- 【Census 数字（全部上断言，WCAG 1.4.11 非文本 3:1 / 小文本 4.5:1）】dark：chip hover 7.96:1、chip rest 5.16:1、strip ring 9.62:1；light：chip rest 4.68:1、chip hover 8.60:1、**strip ring 5.95:1（旧 cyan-500/70 是 1.70:1——普查的捕获修成数字）**。预飞 Python 脚本（oklch→sRGB→WCAG）先行算底牌，探针活体数字与预飞逐位一致。
- 【第二层真相·五】①**在册 ≠ 活体（第 4 度，且观察者自己也会中枪）**：t191 X4 断言了 ring 规则在册，但活体从未被问过「上漆了吗」——而我的第一版断言 regex 抓 boxShadow 的**第一个颜色**，读到的是 Tailwind v4 分层阴影列表里的**透明占位符**（rgba(0,0,0,0)×3 之后才是 ring 层）。修法=读 `--tw-ring-color` 通道：它只在 :focus-visible 真命中时才有值——一个通道同时断言「规则命中」与「颜色归 token」，比正则强两个数量级。②**回执之死是宿主之死**：jump 回执在 300ms 内消失、无任何清除日志——真相是 re-open 后的双 dialog 世界里，退场中的旧 dialog 把自己的面板（连同回执 DOM）一起带走。瞬态 UI 的生命周期以宿主 dialog 的退场为界；探针读瞬态前先读「它的宿主还开着吗」。③**Escape 竞速的前线重演（第 3 度）**：t198/t193 的自愈还是 t196 时代的炮弹（t198 条件化但无闸门、t193 干脆无条件）——回归首跑双双死在「Toggle 按钮 30s 超时」。修法=稳定性双查闸门（250ms 间隔两次可见才按），两套前浪各自复绿。**间歇 flake 的根因清单里，「前浪探针的按键时序假设」这一栏永远填不满**。④**断言的房间标签要与字节同房**：探针没切主题，「CENSUS dark」实测的是 light 字节（hover 8.60/rest 4.68 都是 light 值）——数字全过线，但标签撒谎。t192 的「用系统的门，别用 emulateMedia」教训必须连着「开门」一起带进每一支探针。诚实不是数字对，是数字与房间一一对应。⑤**门链的 dialog 栈是常驻地形**：enlarge 对话框在 viewer 底下从不关——跨栈动作（主题开关在 header）必须先清栈（`dialog-overlay[data-state=open]` 驱动的 Escape 循环），否则 force click 也只是点到 overlay 的纸上。
- 【回归·14 套全绿】t198(34，闸门回迁后)/t200(28)/t191(40，X4 同步后)/t195(40)/t196(29)/t193(29，无条件炮弹拆装后)/t197(50)/t192(14)/t199(18)/t201(37)/t190(29)/t189(42) + 三件套 qa00/qa63/t181 + qa67(27)。**产品侧零行为回归**（chip 文本逐字节、报告字节、pairwise/轮廓全免疫——from/to 是加法，ring 是颜色，button 是升级）。t197 首跑挂 sbatch 按钮超时一次（报告对话框退场动画吃掉 job 点击的时序 flake），复跑即绿——记录在案未加甲。
- 【世界收尾】正典 26 精确保持（Z 相只读证明）；定妆照归档 scripts/shots-t202/：**t202-chip-row-2x.png（chip 行特写——火花线+地址+verdict 住进同一枚按钮）**+ **t202-panel-2x.png（点击后 plane 63% + hover 态 chip + pairwise chip 的活照——门自己的产品）**+ **t202-light-ring-2x.png（浅色房 strip 佩戴 2px 实心环——普查修复的出生证）**+ t202-jump-2x/t202-viewer-2x/t202-light-census-2x（探针活照）；dbg×4 临时脚本删除；t202-shots.mjs 留档（API-staged 世界+回执窗口+归还要齐全）。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「地址是街号，门是动词」：t198 的 chip 教你「去看 50–75% 那段」，t202 的 chip 带你去——QC 动作从「知道去哪」变成「一击即达」，且 2D/3D 两个房间同帧服从。仪器递进的新一格：**可定位 → 可导航**；下一步自然是「导航后自动比对」（跳深后高亮该带的 r 值？留作候选）
- 「机器地址住在词汇旁边，而不是词汇里面」：from/to 是等数切法真实产出的分数界（与名义标签至多差一个平面），label 是人类词汇、from/to 是坐标——报告印词汇、墙上按坐标导航，同一位父亲的两张脸。任何「从 '(50–75%)' 里抠数字」的冲动都是 parse-of-parse 在新轴上的复活
- 「在册 ≠ 活体，且观察者的断言自己也要过堂」：ring 规则在册四窗无人知它从未被活体验证；而验证它的第一支断言又栽在自己的 regex 上（Tailwind v4 阴影列表的第一个颜色是透明占位符）。**断言一个样式状态，要找到「该状态存在时才有值」的通道**（--tw-ring-color 之于 :focus-visible）——通道本身是断言的一半
- 「前浪探针的 Escape 闸门是永远填不满的清单」：t196 发现、t195 补过、t202 又在 t198/t193 两处前线捉到——每支带 popover 的探针都值得一次炮弹审计。稳定性双查（250ms 两次可见）是闸门的最小形状
...
## Task 204 (2026-09-15, cron 13:17/13:47 窗口 trace …202609151317/…1354)

**主题：the territory opens its own DOOR——背叛的领地自己会开门。t198 命名地址（chip）、t202 给 chip 装门、t203 把领地画上景观条、t204 让颜料本身成为门：按下括号，平面落在带的 CENTRE（chip 的同款跳深、同款回执），从不落在点击的 x——门是被按的，不是被刷的，stopPropagation 把 t190 的 scrub 挡在门外。本窗为**接手中断会话**：上会话（13:16 前中断）已落实现（0245d65 cron 自动提交，+16/-3）+ 完成构建，探针/回归/worklog 全缺——判稳后按 t200 判例接手而非另起炉灶。t204 30 断言 ×3 全绿；回归 16 套 + qa 三件套全绿；**沙箱 05:56 文件系统重置判态与 roster oracle 全系同步（26→21，16 文件 36+9 处）**；矩阵 134→135（t204 入册）。**

- 【开局·判态三连】worklog 尾部=Task 203/840da5b（内嵌摘要「Task 198/e490532」**第 14 度过时**——又落后 5 窗）；HEAD=0245d65 领先 origin/main 一个 cron 自动提交——diff 显示 molstar-embed.tsx +16/-3：括号 rect 加 onPointerDown（stopPropagation + applySliceIntent(centre) + flashProfileNote 回执）+ cursor-pointer/hover:opacity-85，注释自称 t204。核实链：文件 mtime 05:13:51 → 构建 05:15:10（chunk 内 grep 到回执字符串「the centre of」）→ 提交 05:16:33——**实现+构建已就位，探针缺位**（无 t204-e2e.mjs），in-flight 判定成立。
- 【判稳·波折】初代 watchdog 被工具会话收割（setsid 无 disown 不够）；重启后 t204 首跑挂「Overlay maps 30s 超时」→ 诊断发现 **server 已死**。重建 watchdog 时与手动 start-prod 互相残杀（watchdog 的 pkill 段杀掉手动实例）——停 watchdog 单次启动。最终配方：`setsid bash scripts/qa-server-watchdog.sh > .qa-logs/... & disown`；且**绝不用管道接 start-prod**（head 关闭管道时 setsid 的 bun 收 SIGPIPE 死亡——诊断命令自己杀了 server）。
- 【沙箱重置判态·本窗最大考古】重建构建时发现 .next 主产物消失、.next/dev（05:56）出现、db/custom.db 清零——**05:56 文件系统级重置**（三证据同一时刻）。旧世界（roster 26）中 5 个 job（t182 census 的 4 idle+1 failed）只活在旧文件系统、无 seeder 可考——不可恢复。restore-gallery 重建出 21（3 demo + 11 skeleton + 2 qa58 + 5 qa60）。**判决：非产品 bug，全系 oracle 26→21**（t202「前浪 oracle 跟世界走」判例）：t204-sync-roster-oracle.py 36 处/16 文件命中 + t182 手工 9 处变体（B4 edges 以 wire 实测 16 为准非 DB 的 15——**S7 的「15 DB + 1 file-only edge」注释早已预言这条差异**；census failed===0 改 (?? 0)——0 failed 时 JSON 键缺席）。
- 【实现·已由上会话完成（0245d65）】①centre = Math.round(((w.from+w.to)/2)*100)/100——与 chip 门逐字节同算法（X3 断言恰好 2 次）；②回执模板与 chip 逐字节同句（X4 恰好 2 次）；③onPointerDown = stopPropagation + applySliceIntent({pos: centre})——ONE father 第三位门客，2D 正交面板同帧跟随免费获得；④cursor-pointer + transition-opacity + hover:opacity-85（hover 语义进 class list）；⑤aria-hidden 幸存——门只回答手，chip 仍是键盘的门。
- 【探针 t204·30 断言 ×3 全绿】S（seeder+halves）+ W（band oracle + centre 算术：half1 Q3 centre 0.63、half2 Q1 centre 0.13）+ X7 源 oracle（门方言与 chip 同构恰好 2 次、scrub 父亲幸存、aria-hidden 幸存、t203 属性幸存）+ D 相活线（**D7 核心一帧：按下括号 15% 处落点==13% 而非 ~4%——门赢 scrub 在同一按压里**；D10 反方向 85% 处落点==63% 非 ~71%；回执逐字节 ×2；visiting 随门翻转；**D12 hover 三态 rest 0.3→hover 0.85→leave 0.3**；D13 aria-hidden 层零 tabindex；**D14 键盘门幸存：Enter on chip 仍落 63%——括号的门什么也没吃**）+ Z（roster 恒等、console 零错）。
- 【第二层真相·五】①**观察者的手指自己携带状态**：D12 首跑 rest 读到 0.85——D10 的按压把指针留在了括号上，hover 类持续点亮；「先把手挪开再读静止」是 t201 输入模态教义的手之版本。②**visiting 翻转是重渲染不是按键回执**（t200「落袋在 wire 上」的绘画版）：Home 后盲睡 700ms 偶发不够——轮询到 attr 变 0.3 才是合同，RUN=3 的输赢就在这一拍。③**守护数字必须敢于跟随世界的重生**：26 是历代「正典精确保持」的基石，但沙箱重置后它守护的是一个不存在的世界——oracle 的铁律部分（本轮恒等、世界不漏）与基线部分（具体数字）要分开对待；基线死了就诚实换新，绝不造假 job 凑数。④**wire 是边数的唯一真理**：DB 里数出 15 条 edge，wire 报 16——t182 S7 的「15 DB + 1 file-only」注释早已预言；API 直查永远少一条看不见的边。⑤**pipeline 的每一环都可能反噬**：head 杀 bun（SIGPIPE）、watchdog 杀手动 server（pkill 段）、构建产物随沙箱蒸发——启动配方的一条注释就是一次重生，`setsid + disown + 重定向 + 无管道` 是本窗的完整护身符。
- 【回归·16 套 + 三件套全绿】t190(29)/t203(27)/t182(76，oracle 同步后)/t181(23)/t198(35)/t200(28)/t202(42)/t199(18)/t201(37)/t192(14)/t191(40)/t189(42)/t195(40)/t196(29)/t193(29)/t197(50) + qa00/qa63/qa67(27)。产品侧零行为回归——门是加法（rect 的事件处理器），括号几何/visiting/chip 文本/轮廓/scrub 全免疫。
- 【世界收尾】定妆照归档 scripts/shots-t204/：**t204-panel-jump-2x.png（一帧三仪器：括号点亮@63% + chip visiting 墨色 + 回执命名门——门自己的产品）** + **t204-chip-row-2x.png（chip 行特写：half1 青色火花线亮墨 vs half2 紫色灰调，对照分明）** + t204-door-2x/t204-viewer-2x（探针活照）；dbg 脚本删除；t204-shots.mjs 留档（API-staged 世界+全链重入+归还要齐全）；t204-sync-roster-oracle.py 留档（oracle 迁移的可重放记录）。
- 【收尾】worklog（本条）+ commit + push + 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「地址 → 门 → 领地 → 门内的门」：t204 后 QC 仪器完成最后一级闭环——背叛在哪（chip）、怎么去（点击）、疆土多大（括号）、疆土本身就是门（按一下落到带心）。景观条现在是完整的仪器：地形（r 分布）+ 领地（weakest band）+ 门（点击即达）+ 播报（回执），四个部件共享同一位数学父亲
- 「门赢 scrub 在同一按压里」：stopPropagation 不是防御是**语义分界**——括号内的像素说「跳到带心」，括号外的像素说「scrub 到我这里」；一条 pointerdown 事件把景观条切成两种动词，而 2D 面板对两者都同帧服从（ONE intent event）
- 「守护数字与铁律要分开安葬」：roster oracle 的铁律（本轮恒等、名字不漏）万岁，基线数字（26）随旧文件系统殉葬——诚实的 oracle 敢于改数字，绝不用假数据维持旧数字；t204-sync-roster-oracle.py 把 36 处迁移做成可重放脚本，迁移本身也要有据可查
- 「观察者的手指也是观察对象」：D12 的 0.85 不是产品的谎，是探针自己的手指忘了走——hover 态的世界里「静止」是把手挪开之后才存在的事实；同理 visiting 翻转要轮询到画布真变，盲睡是对重渲染的赌注
- 世界卫生观察账本（verdict 列）：本轮无全矩阵（单仪器轮，183 判例=定向回归覆盖 viewer+strip+chip+克隆面 16 套）；t204 ×3 + 回归 16/16 + 三件套全绿；**roster oracle 基线 26→21（沙箱重置判决，全系同步）**；矩阵显式清单 134→135（t204 入册）
- 遗留（下轮候选）：报告 Local 表加 from/to 百分比列（词汇旁边印坐标——chip 的机器地址同源，t202 教义的报告章）；grabber nudge/undo 手感参数（真机盲区依旧）；script RELION-present 分支（沙箱受限判决维持）；EMPIAR 真数据回归（让位）；runner wall-time 剖面（让位）；qa77 的孤儿 job 测试在 21 基线下需复核（本轮未跑，其 seeder 依赖 demo 原生 3 idle——现状仍在）

## Task 205 (2026-09-15, cron 16:02/16:47 窗口 trace …202609151602/…1654)

**主题：the paper learns the address's DIALECT——报告把坐标印在词汇旁边。t198 给报告最弱带的名字（Q3 (50–75%)），t202 让墙按真实 fraction 范围导航，t204 让颜料本身成为门；t205 闭合教义的纸面一侧：Local agreement 表长出尾列「Depth (fraction)」，逐行引用该图最弱带的 `from–to`——与 viewer 括号门跳向的同一组数字，用景观条自己的 2dp 方言，追加在 Weakest 列之后（前浪每个格索引原位不动，t198 的 D7 读 cells[2..6]、`>= 7` 长度守卫早就在等这一列）。flat 裁决没有地址，格子保持诚实的「—」。本窗从跨窗考古开始：内嵌摘要第 15 度过时（宣称 Task 204 卡在写探针），实际远程 7ac8865 已含 t205 in-flight（qc-report.ts 实现 + 286 行探针 + dbg 脚本）——以 t200 判例接手而非另起炉灶。t205 36 断言 ×3 全绿；回归 8 套全绿（t198/t202/t203/t204/qa00/qa63/t181 + t205 自身）；矩阵 135→136（t205 入册）。**

- 【开局·跨窗考古三折】①worklog 尾部=Task 204、本地 HEAD=6aafba6（06:53 Task 204 完成提交）——但 push 被拒：**远程 origin/main=7ac8865 与本地 6aafba6 是同一 Task 204 的两个副本**，且 `git diff 6aafba6 7ac8865` 显示远程树**缺** 5 个文件（t205-e2e.mjs 286 行、qc-report.ts ±18、dbg-t205.mjs、2 张 diag 图）——即本地旧副本才是带 t205 in-flight 的那一支。`git reset --hard origin/main` 对齐后 `git checkout 6aafba6 -- ...` 逐件复活（reflog 考古，零丢失）。②server 表面 LISTEN 实则僵死（curl 000）——杀掉后发现 **.next 构建产物又一次蒸发**（无 BUILD_ID、无 standalone/server.js，05:56 重置的回声）；setsid 后台 build 静默死亡（进程收割判例再添一笔：**build 也要前台跑**），前台 `bun run build` 一次通过。③世界第三次清零（QA Refine3D 缺失）——restore-gallery 重建 21。
- 【实现·上会话遗产（本轮只验证未改动）】①表头加 `| Depth (fraction) |` + 7 列分隔行；②`depth = w ? \`${w.from.toFixed(2)}–${w.to.toFixed(2)}\` : "—"`——**直接饮 from/to**，从不像 X5 禁止的那样解析词汇；③prose 尾句改名：「the Depth column quotes that address as fractions (the same numbers the viewer's bracket door jumps to)」；④LocalBand 契约注释加 t205 章：「the paper and the wall now quote the same address, each in its own dialect」。
- 【探针 t205·36 断言 ×3 全绿（装甲五连）】S（seeder + orthovol + 双 half）+ **S3 卫生起点**（开页前 PUT overlay-session 空——服务端镜像跨浏览器跟随 job，持久 session 让 Mol* 在按钮出现前恢复 MRC，冷盒子能把按钮藏几分钟；playwright 新 context 无 localStorage，浏览器镜像天然干净）+ W（band oracle + depthStr：half1 Q3 0.50–0.75、half2 Q1 0.00–0.25）+ X6 源 oracle（7 列表头、depth 直饮 from/to、Weakest 方言逐字节、禁字符串手术、契约注释）+ D17 活线（**D7/D8 深度格与 wire oracle 逐字节**、D9/D10 Weakest 格 == t198 oracle 原位、D11 四个 quarter 格仍等于探针自己的 oracle——表身在新列旁幸存、D5 表头 7 列、D6 每行 9 段（2 边界空 + 7 列））+ Z3（roster 恒等 21、console 零错）。
- 【装甲五连·每连一个判例】①**点击重试**（t195 两度即装甲）：outputs 扫描重渲染会 detach 按钮 mid-click（RUN=1 目击「element is not stable/detached」）——bounded 3 次尝试 + 失败后 Escape+re-open+re-poll。②**冷会话自清**（RUN=4/5）：持久 session 的恢复窗口把 Overlay maps 按钮藏过 90s/150s——dbg2 实证「session-free 世界按钮 ~5s 即现」；探针先给按钮 45s，缺席则 PUT 双镜像清空 + reload + 重走全链。③**profile 面板先行**（RUN=6 的假阴性教训）：chip 住在 profile 面板的 local row 里，面板不开则一切 chip 查询都是假阴性——D2 的 count===0 曾因错误理由通过；t204 的次序（Toggle cross-section plane → strip 可见 → 再 adopt）补进 t205。④**terrain custody**（t198 D4 信号转正）：overlay terrain 是 Mol* 自己的完成收据——地图加载+渲染完才出现，主线程随即空闲；RUN=9 证明在 Mol* 吞 half1 的 MRC 时点 half2 会把页面冻过任何诚实超时——采纳串行在渲染器的真实就绪上。⑤**double-toggle 陷阱**（RUN=10）：面板按钮是 toggle，面板开着再点=关闭——ensureOpen 先看后点（choice 已可见即面板已开），点击重试的 catch 也走 ensureOpen。⑥**导出触发**（t198 契约）：buildProfileReport 只在 copy/download handler 里跑——carrier 的 data-md 在按下 Copy 按钮前恒为 null（RUN=8 在等一个永远不会自己出现的值）。⑦**chip 长轮询**（RUN=3/6/7 定论）：profile effect 每次采纳会重置并重取**所有** overlay 的 terrain，chip 可滞后点击数分钟——D3 统一 200s 长轮询裁决，无固定小睡能在冷 MRC 读取下幸存。
- 【第二层真相·六】①**同一地址的两种方言**：墙上的 chip/括号/门引用 0.50–0.75，纸上的 Depth 列引用 0.50–0.75——数据同源（localAgreement+weakestBand 的第四位消费者），方言各异（墙跳转、纸引用）；仪器的第五格还是不需要新数学。②**探针的时序学是产品的时序学的镜像**：七个 flake 位点（扫描重渲染、会话恢复窗口、面板不开、Mol* 主线程、toggle 双跳、导出前 null、chip 滞后）没有一个是产品 bug——每一个都是「UI 完成的真实时刻」与「探针以为的时刻」之差；装甲不是容错是对产品完成语义的精确描述。③**前浪的守卫是预言**：t198 的 `>= 7` 长度守卫「早就在等这一列」——探针写守卫时想象过的未来列，三个任务后真的来了；前浪断言的每一分冗余都是给后浪的路权。④**假阴性比失败更危险**：D2 的 count===0 曾因「面板没开」而非「世界干净」通过——一个断言可以通过错误理由变绿；armor 之后必须问「这个绿是我想的那个绿吗」。⑤**reflog 是跨窗传承的保险柜**：reset --hard 之后 6aafba6 仍在 reflog 里，`git checkout <sha> -- <path>` 逐件复活——「丢弃」在 git 里从来不是删除，是暂存。⑥**build 与 server 同为收割对象**：setsid 后台 build 静默死亡（无日志无进程），前台 build 一次通过——收割者不区分长任务种类，凡后台皆风险。
- 【回归·8 套全绿】t198(35——报告章主，cells 索引免疫实证)/t202(42)/t203(27)/t204(30)/t181(23)/qa00(data-view sentinel)/qa63(smoke) + t205 ×3 自身。产品侧零行为回归——Depth 列是加法（表尾新格），Weakest 词汇/quarter 格/章节边界/prose 全免疫。
- 【世界收尾】定妆照 scripts/shots-t205/t205-paper-2x.png（报告全景：7 列 Local 表 + Depth 格 + prose 收尾句）；dbg-t205/dbg2-t205 与 diag 图按判例删除；run-matrix.sh t205 入册（135→136）。
- 【收尾】worklog（本条）+ commit + push（带上 e45be0d 的 in-flight 遗产）+ 环境清理（杀 server 先 ss 查真实 PID、杀 watchdog、agent-browser close）。

Stage Summary:
- 「地址的第四位消费者」：from/to 这个数字从 t202 出生（chip 的跳深坐标），t203 画成括号，t204 变成门，t205 印上纸面——四个消费者共享同一位数学父亲，每个新仪器都是旧真话的又一种读法；报告与墙现在引用同一个地址，各有各的方言（墙跳转、纸引用、括号点亮、chip 播报）
- 「完成有七个时刻」：扫描结束、面板打开、地图加载、terrain 渲染、profile 取回、报告导出、chip 现身——七个时刻各有一次「探针以为完成了」的陷阱；t205 的七连装甲是产品完成语义的说明书，比任何一次盲睡都精确
- 「假阴性审判」：count===0 的绿必须追问理由——D2 曾因面板没开而「证明」世界干净；断言的语义不在于它的布尔值，在于它的前提链
