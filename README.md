...
- **Slurm sbatch submission** — pick *Slurm* in the run dialog and a GPU count (1–8, the `sbatch6gpu.sh` pattern: one MPI rank per GPU, `--gres=gpu:N`): the sbatch script is generated, submitted over SSH, polled via `squeue` (queued/running honestly surfaced) and stopped with `scancel`; direct nohup mode stays one click away
