...
- **Remote projects (data stays on the cluster)** — a project's Data location binds it to a saved SSH login: the import/STAR-file browser walks the CLUSTER's filesystem over SSH (roots, listings, wildcard patterns), imports write cluster-absolute paths (zero upload — not one movie byte moves), and the primary Run button dispatches straight to the bound cluster ([docs](./docs/remote-relion.md#4c-local-vs-remote-projects))
