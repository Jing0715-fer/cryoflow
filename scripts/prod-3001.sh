#!/usr/bin/env bash
# CryoFlow standalone PROD server on :3001 (t301 doctrine, t311 polish):
# the dev server's Turbopack side holds ~3.4GB RSS on this 4GB box and the
# OOM killer wins any browser+dev coexistence; the standalone build runs in
# a few hundred MB. Orphan-launch pattern (survives the tool-call reaper:
# script exits immediately, server re-parents to init).
