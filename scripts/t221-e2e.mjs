...
must(JSON.stringify(headCells) === JSON.stringify(["Job", "Main map", "Volumes", "Peak", "Δ winner", "Agreement r"]),
  `R1 the head is the sextet (${JSON.stringify(headCells)})`);
const rows = await page.locator("[data-report-body] tr[data-owner-door]").count();
must(rows >= 2, `R2 the roster speaks (${rows} owner rows)`);
const rCells = (await page.locator("[data-report-body] tr[data-owner-door] td:last-child").allTextContents()).map((c) => c.trim());
