/* t209 e2e — the dialect learns to INTRODUCE ITSELF. t203 drew the
 * betrayal's territory, t204 made it a door, t205/t206 put the address on
 * the paper (Local + Pairwise Depth columns), t207/t208 gave every chip
 * and every bracket its own door — but a newcomer staring at the panel
 * had NO WAY to learn what any of it means: the fractions, the
 * equal-count knife, the neutral pair ink, the doors, or WHY the
 * Comparison table carries no Depth column (its address lives in Local —
 * a depth printed twice has two fathers and drifts apart). t209 gives
 * the dialect a voice: an opt-in legend (amber, the teaching door's own
 * accent against the CSV's cyan and the report's violet) that names every
 * territory the strip draws WITH THE INSTRUMENT'S OWN INK (the swatches
 * are the overlays' real colours, the same hex the brackets wear), and a
 * paper epilogue (### Reading the depth addresses) so the exported report
 * teaches the same dialect to readers who never open the app. Both
 * surfaces speak ONE vocabulary; the legend's territory rows mirror the
 * world exactly (zero overlays = no overlay rows, no pair row — the
 * honest absence is live-testable at BOTH ends).
 *   S  setup — seeder, orthovol + BOTH half-maps (+ hygienic start)
 *   W  wire — band oracles + the PAIRWISE oracle + the door's centre
 *   X  source oracles — t208's full set + the legend's door (aria
 *      semantics, amber teaching accent, real-ink swatches) + the
 *      epilogue (guarded like the tables it teaches, before the footer)
 *   D  live — t208's full set re-green + the legend: default closed,
 *      open teaches, territory rows mirror the world (2 pre-adoption,
 *      5 after), swatch ink == strip bracket ink, the doors still answer
 *      with the lesson showing, and the epilogue rides the copied report
 *      in the right place (after Pairwise, before the export footer)
...
// ============ t209: the legend, live with BOTH maps speaking ============
// the lesson re-opens AFTER the adoptions: now the territory rows must
// mirror the FULL world (landscape + two overlay rows + pair row +
...
must((terr2.landscape ?? 0) === 1 && (terr2.overlay ?? 0) === 2 && (terr2.pair ?? 0) === 1 && (terr2.playhead ?? 0) === 1, `D52 the territory rows mirror the FULL world (landscape 1, overlay 2, pair 1, playhead 1 — got ${JSON.stringify(terr2)})`);
must((await legBlock.locator('[data-legend-territory="overlay"][data-legend-map="run_it020_half1"]').count()) === 1 && (await legBlock.locator('[data-legend-territory="overlay"][data-legend-map="run_it020_half2"]').count()) === 1, "D53 each overlay's row is NAMED (data-legend-map == the map's own name, not a paraphrase)");
...
for (const h of ["run_it020_half1", "run_it020_half2"]) {
...
must((await legBlock.locator('[data-legend-territory="pair"]').textContent()).includes("wearing neither's colour"), "D55 the pair row teaches the neutral ink's meaning (corroboration belongs to the pair)");
...
await page.screenshot({ path: `${OUT}/t209-legend-open-2x.png` }).catch(() => {});
...
await page.screenshot({ path: `${OUT}/t209-final-2x.png` }).catch(() => {});

/* ============ Z: read-only ============ */
section("Z: the world read back");
const roster1 = (await (await fetch(BASE + "/api/jobs")).json()).jobs ?? [];
must(roster1.length === roster0.length && roster1.length === 23, `Z1 roster identity (${roster1.length}, canonical 23)`);
