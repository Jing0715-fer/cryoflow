/* t205 e2e — the paper learns the address's DIALECT. t198 gave the
 * report the weakest quarter's NAME (Q3 (50–75%)), t202 taught the wall
 * to navigate by the ACTUAL fraction extent (from/to), t204 made the
 * paint a door; t205 closes the PAPER side of that doctrine: the Local
 * agreement table grows a trailing "Depth (fraction)" column quoting
 * each map's weakest band as `from–to` — the same numbers the viewer's
 * bracket door jumps to, in the strip's own 2dp dialect, appended AFTER
 * the Weakest column so every front-wave cell index survives untouched
 * (t198's D7 reads cells[2..6], its `>= 7` length guard was already
 * waiting for this column).
 *   S  setup — seeder, orthovol + BOTH half-maps
 *   W  wire — the probe's own band oracle + the DEPTH string math
 *   X  source oracles — the table carries 7 columns, the depth cell
 *      drinks from/to, a flat verdict keeps the honest dash, the prose
 *      names the column, and NOTHING parses the vocabulary
 *   D  live — export the report, cut the Local section, compare the
 *      header and both depth cells BYTE-FOR-BYTE against the wire
 *      oracle; the Weakest cell (cells[6]) is byte-identical to t198's
 *      oracle (the front wave survives, live, in the same frame)
...
const d1 = depthStr(w1), d2 = depthStr(w2);
console.log(`  (oracle: half1 ${QUARTERS[w1.k]} depth ${d1}; half2 ${QUARTERS[w2.k]} depth ${d2})`);
...
await page.screenshot({ path: `${OUT}/t205-paper-2x.png` }).catch(() => {});
