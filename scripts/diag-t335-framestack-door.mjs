...
try {
  ...
  // the mixed import's star copy exists locally (stars always sync) —
  // remove it so the dispatch resolves the input to the CLUSTER twin, the
  // exact shape t334 used to skip with a console note. The refusal must
  // still speak, this time over the SSH-read star.
  const importStarLocal = `${ROOT}/data/relion/${projectId}/import_${importA.id.slice(-8)}/micrographs.star`;
  must(existsSync(importStarLocal), "the import star's local copy exists (stars always sync home)");
  let starBackup = null;
  try {
    starBackup = readFileSync(importStarLocal, "utf8");
  } catch {
    starBackup = null;
  }
  rmSync(importStarLocal, { force: true });
  must(!existsSync(importStarLocal), "the local star copy is gone — the dispatch must resolve the cluster twin");

  const dEX2 = await dispatch(extractA.id, {
    remote: { connectionId: CONN, module: MODULE, mode: "slurm" },
  });
  const err2 = String(dEX2.body?.error ?? "");
  must(
    !!dEX2.body?.error,
    `B3 the twin-star refusal still speaks (body: ${JSON.stringify(dEX2.body ?? {}).slice(0, 240)})`
  );
  ...
  // restore the local copy for the teardown's hygiene
  if (starBackup !== null) {
    const { writeFileSync } = await import("node:fs");
    writeFileSync(importStarLocal, starBackup);
  }
