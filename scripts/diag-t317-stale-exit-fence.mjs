#!/usr/bin/env node
/**
 * t317 diag — the re-run's ghost: a stale .cf-exit must never forge the
...
const plantGhost = (workdir, { backdate = false } = {}) => {
  const out = clientBoth(
    `mkdir -p ${workdir}; echo 1 > ${workdir}/.cf-exit; ` +
      ...
      `echo 'CRYOFLOW_NOTE: GHOST-LOG-t317 from the previous failed run' > ${workdir}/run.out; ` +
...
try {
  ...
  const proj = await api("/api/projects", {
    method: "POST",
    headers: SHJ,
    body: JSON.stringify({ name: "QA t317 ghost fence", mode: "remote", remoteConnectionId: CONN }),
  });
  ...
  const fixtureOut = clientBoth(
    "mkdir -p /data2/ghost-t317; " +
      `echo ${singleB64} | base64 -d > /tmp/.t317-single.mrc; ` +
      "for i in $(seq 1 8); do cp /tmp/.t317-single.mrc /data2/ghost-t317/20241031_lijing_${i}_Fractions_DW.mrc; done"
  );
  must(fixtureOut === "", `the fixtures build quietly (${fixtureOut.slice(0, 120)})`);
  const nMrc = client("ls /data2/ghost-t317/*.mrc 2>/dev/null | wc -l");
  ...
  const fileList = Array.from(
    { length: 8 },
    (_, i) => `/data2/ghost-t317/20241031_lijing_${i + 1}_Fractions_DW.mrc`
  );
  const importA = await mkJob({
    projectId,
    type: "import",
    name: "QA t317 remote mics",
  ...
  const ctfA = await mkJob({ projectId, type: "ctffind", name: "QA t317 ctf ghost", params: {} });
  ...
  must(!outA.includes("GHOST-LOG-t317"), "the ghost's poisoned run.out never resurfaces");
  ...
  // telling the truth.
  const ctfB = await mkJob({ projectId, type: "ctffind", name: "QA t317 ctf fence", params: {} });
  ...
  const ctfC = await mkJob({ projectId, type: "ctffind", name: "QA t317 ctf direct", params: {} });
  ...
  must(!outC.includes("GHOST-LOG-t317"), "the direct ghost's poisoned log never resurfaces");
  ...
  console.log(`\n== t317 diag: ${fail === 0 ? "ALL GREEN" : `${fail} FAIL`} ==`);
} finally {
  ...
  try {
    client("rm -rf /data2/ghost-t317 /tmp/.t317-single.mrc");
